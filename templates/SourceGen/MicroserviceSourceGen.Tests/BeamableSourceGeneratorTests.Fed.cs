...
";
	...
	[Fact]
	public async Task Test_Diagnostic_Fed_FederationConfigFileNotFound()
	{
		const string UserCode = @"
using Beamable.Server;
using Beamable.Common;
...
";
		...
		ctx.ExpectedDiagnostics.Add(new DiagnosticResult(Diagnostics.Fed.FederationConfigFileNotFound));
	...
	[Fact]
	public async Task Test_Diagnostic_Fed_ErrorParsingFederationConfigFile()
	{
		const string UserCode = @"
using Beamable.Server;
using Beamable.Common;
...
";
		...
		ctx.ExpectedDiagnostics.Add(
			new DiagnosticResult(Diagnostics.Fed.ErrorParsingFederationConfigFile).WithArguments(
...
";
		var cfg = new MicroserviceFederationsConfig() { Federations = new() { { "notdefault", [new() { Interface = "IFederatedPlayerInit" }] } } };
		var ctx = new CSharpCodeFixTest<ServicesAnalyzer, FederationDefaultIDFixer, DefaultVerifier>();
...
";
		var cfg = new MicroserviceFederationsConfig() { Federations = new() { { @"123my_f", [new() { Interface = "IFederatedGameServer" }] } } };
		var ctx = new CSharpCodeFixTest<ServicesAnalyzer, FederationIdNameFixer, DefaultVerifier>();
...
";
		...
		var ctx = new CSharpCodeFixTest<ServicesAnalyzer, FederationDefaultFixer, DefaultVerifier>();
