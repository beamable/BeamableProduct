...
public static class Diagnostics
{
	...
	public static class Srv
	{
		...
		public static readonly DiagnosticDescriptor CallableTypeInsideMicroserviceScope
			= new(CALLABLE_METHOD_TYPE_INSIDE_MICROSERVICE_SCOPE_ID,
				...
				$"{nameof(Server.Microservice)} callable method '{{0}}' uses type '{{1}}', which is not allowed in the microservice scope. Ensure all types used in microservice callable methods are defined in a shared project that both client and microservice can reference, or mark {{0}} to skip during Client Code Gen. You can either move {{1}} type to Assets/Beamable/Common or any other shared lib or add the callable flag `CallableFlags.SkipGenerateClientFiles` to {{0}} method.",
				Category_Services,
				DiagnosticSeverity.Error,
	...
	public static class Logs
	{
		...
		public const string LOGGING_GUIDE_BASE_URL = "cli/guides/ms-troubleshooting/";
		...
		public static readonly DiagnosticDescriptor NonConstantLogTemplate
			= new(NON_CONSTANT_LOG_TEMPLATE_ID,
				...
				Category_Logging,
				DiagnosticSeverity.Warning,
				helpLinkUri: DocsPageHelper.GetCliDocsPageUrl($"{LOGGING_GUIDE_BASE_URL}#log-message-template-must-be-a-constant", Constants.CLI_CURRENT_DOCS_VERSION),
				isEnabledByDefault: true);
		...
		public static readonly DiagnosticDescriptor UnrenderableLogTemplate
			= new(UNRENDERABLE_LOG_TEMPLATE_ID,
				...
				Category_Logging,
				DiagnosticSeverity.Error,
				helpLinkUri: DocsPageHelper.GetCliDocsPageUrl($"{LOGGING_GUIDE_BASE_URL}#log-message-template-cannot-be-rendered", Constants.CLI_CURRENT_DOCS_VERSION),
				isEnabledByDefault: true);
	}
