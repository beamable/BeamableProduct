...
namespace Beamable.Common
{
    ...
    public static partial class ReflectionCacheExtensions
    {
        ...
        public static List<AttributeValidationResult<T>> GetAndValidateAttributeExistence<T>(this IReadOnlyList<MemberInfo> membersToCheck,
            AttributeOfInterest attributeOfInterest,
            Func<MemberInfo, AttributeValidationResult<T>> validateOnMissing,
            List<MemberAttributePair> foundAttributes)
            where T : Attribute, IReflectionCachingAttribute<T>
        {
            ...
            var type = membersToCheck.ToList();
            var validationResults = new List<AttributeValidationResult<T>>();
            foreach (var checkType in type)
            {
                var attribute = checkType.GetCustomAttribute(attributeOfInterest.AttributeType, false);
                if (attribute != null)
                {
                    var cast = (T) attribute;
                    var result = cast.IsAllowedOnMember(checkType);

                    validationResults.Add(result);
                    if (result.Type == ReflectionCache.ValidationResultType.Valid)
                        foundAttributes.Add(new MemberAttributePair(checkType, attribute));
                }
                else
                {
                    var result = validateOnMissing?.Invoke(checkType);
        ...
        /// <summary>
        /// Helper that splits a list of <see cref="AttributeValidationResult{T}"/> between <see cref="ReflectionCache.ValidationResultType.Valid"/> and non-<see cref="ReflectionCache.ValidationResultType.Valid"/> results.
        /// </summary>
        public static void SplitValidationResults<T>(this IReadOnlyList<AttributeValidationResult<T>> mainList,
            ...
            out List<AttributeValidationResult<T>> notValid)
            where T : Attribute, IReflectionCachingAttribute<T>
        {
            ...
            notValid = splitByType
                .Where(group => group.Key == ReflectionCache.ValidationResultType.Warning || group.Key == ReflectionCache.ValidationResultType.Error)
                .SelectMany(group => group)
                .ToList();
        }
