...
namespace Beamable.Common.Api.Inventory
{
	...
	public class InventoryResponse
	{
		...
		private HashSet<string> ResolveMergeScopes(InventoryView view)
		{
			...
			resolved.UnionWith(
				view.currencies.Keys.Where(currencyType => SetContainsPrefixOf(scopesLookup, currencyType)));
						...
						foreach (var prop in item.properties)
						{
							if (properties.ContainsKey(prop.name))
							{
								BeamableLogger.LogWarning(
									$"Inventory item has duplicate key. Overwriting existing key. item=[{itemGroup.id}] id=[{item.id}] key=[{prop.name}]");
							}
	...
	public class InventoryView
	{
		...
		public Dictionary<string, List<CurrencyProperty>> currencyProperties =
			new Dictionary<string, List<CurrencyProperty>>();
