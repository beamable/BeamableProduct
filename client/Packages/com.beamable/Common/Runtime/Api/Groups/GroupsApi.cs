...
namespace Beamable.Common.Api.Groups
{
	public class GroupsApi : IGroupsApi
	{
		                                           ...
		                                           int? limit = null)
		{
			...
			{
				args = AddQuery(args, "name", name);
			}

			if (offset.HasValue)
			{
				args = AddQuery(args, "offset", offset.ToString());
			}

			if (limit.HasValue)
			{
				args = AddQuery(args, "limit", limit.ToString());
			}

			if (enrollmentTypes != null)
			{
				args = AddQuery(args, "enrollmentTypes", string.Join(",", enrollmentTypes.ToArray()));
			}

			if (hasSlots.HasValue)
			{
				args = AddQuery(args, "hasSlots", hasSlots.Value.ToString());
			}

			if (scoreMin.HasValue)
			{
				args = AddQuery(args, "scoreMin", scoreMin.Value.ToString());
			}

			if (scoreMax.HasValue)
			{
				args = AddQuery(args, "scoreMax", scoreMax.Value.ToString());
			}

			if (!string.IsNullOrEmpty(sortField))
			{
				args = AddQuery(args, "sortField", sortField);
			}

			if (sortValue.HasValue)
			{
				args = AddQuery(args, "sortValue", sortValue.Value.ToString());
			}
