...
namespace Beamable.Common.Api.Mail
{
	public abstract class AbsMailApi : IMailApi
	{
		...
		public async Promise<SearchMailResponse> SearchMail(SearchMailRequest request, bool includeAuthHeader = true)
		{
			...
			var converted = Utility.Convert(apiRes);
			RememberAttribution(converted);
			return converted;
		}
		...
		public Promise<EmptyResponse> Update(MailUpdateRequest updates)
		{
			return Requester.Request<EmptyResponse>(
			  ...
			  updates
			).Then(_ => ReportOpened(updates));
		}

		/// <summary>
		/// Attribution (<c>beam_outreach</c> / <c>trackId</c>) for mail this context has fetched, so an
		/// Unread -> Read transition can be reported without the caller passing the mail back in.
		/// Bounded, because a long-lived session could otherwise page through unlimited mail.
		/// </summary>
		private readonly Dictionary<long, MailMessage> _attributed = new Dictionary<long, MailMessage>();

		private const int MaxRememberedAttributions = 256;

		private void RememberAttribution(SearchMailResponse response)
		{
			foreach (var clause in response?.results ?? new List<SearchMailResponseClause>())
			{
				foreach (var mail in clause?.content ?? new List<MailMessage>())
				{
					// Only campaign mail is worth remembering; ordinary game mail carries nothing to report.
					if (mail?.metadata == null || !mail.metadata.ContainsKey(OutreachKey)) continue;

					if (_attributed.Count >= MaxRememberedAttributions && !_attributed.ContainsKey(mail.id))
					{
						// Oldest-ish eviction: enumeration order is insertion order in practice, and an
						// exact LRU is not worth a second data structure for a best-effort metric.
						foreach (var stale in new List<long>(_attributed.Keys))
						{
							_attributed.Remove(stale);
							if (_attributed.Count < MaxRememberedAttributions) break;
						}
					}

					_attributed[mail.id] = mail;
				}
			}
		}

		/// <summary>
		/// Reports an Opened for every mail this update moved to Read that carries campaign attribution.
		/// <para>Email and in-game have no handset to echo engagement back the way push does, so for
		/// in-game mail this client report IS the Opened signal. It is emitted on the state transition
		/// rather than on display, so the client's count and the server's Unread -> Read agree.</para>
		/// <para>Best-effort and fire-and-forget by design: a failed analytics call must never make a
		/// mail look unread.</para>
		/// </summary>
		private void ReportOpened(MailUpdateRequest updates)
		{
			if (_attributed.Count == 0) return;

			foreach (var entry in updates?.updateMailRequests ?? new List<MailUpdateEntry>())
			{
				if (entry?.update == null || entry.update.state != MailState.Read.ToString()) continue;
				if (!_attributed.TryGetValue(entry.id, out var mail)) continue;

				// Once only: the server dedups on (outreachId, stage) too, but not re-sending is cheaper
				// and keeps a re-read from looking like traffic.
				_attributed.Remove(entry.id);
				...
				{
					// Swallowed: no analytics service registered, or it threw. Not worth a broken mailbox.
				}
			}
		}

		/// <summary>Wire keys the message rail stamps onto campaign mail metadata.</summary>
		private const string OutreachKey = "beam_outreach";
					...
					res.results.Select(r => new SearchMailResponseClause
					{
						...
						content = r.content.HasValue ? r.content.Value.Select(c =>
						{
							...
							return new MailMessage
							{
								...
								expires = expires,
								rewards = rewards,
								metadata = c.metadata.HasValue && c.metadata.Value != null
									? new SerializableDictionaryStringToString(c.metadata.Value)
									: null
							};
	...
	public class MailMessage
	{
		...
		/// <summary>
		/// Opaque per-message metadata set by whoever sent the mail. A campaign message rail stamps its
		/// attribution here (<c>beam_outreach</c> / <c>trackId</c>), which is what lets
		/// <see cref="AbsMailApi.Update"/> report an Opened for it.
		/// Null when the sender set none, which is the normal case for ordinary game mail.
		/// </summary>
		public SerializableDictionaryStringToString metadata;
