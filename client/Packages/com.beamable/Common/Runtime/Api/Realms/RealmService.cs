...
namespace Beamable.Common.Api.Realms
{
	...
	public class RealmsService : IRealmsApi
	{
		...
		{
			// Uses the customer-actor endpoint (GET /api/customers/{customerId}) through the generated
			// BeamCustomerApi. It's constructed inline from the requester this service already holds so
			// that this singleton doesn't take a scoped dependency on the generated api. The customer id
			// is the current cid on the requester.
			var customerApi = new BeamCustomerApi(_requester);
			return customerApi.Get(_requester.Cid)
							 .Map(view => new CustomerView
							 {
								 Cid = view.customerId,
								 Alias = view.alias.GetOrElse((string)null),
								 DisplayName = view.name,
								 Projects = ProcessProjects(ToProjectViews(view.realms))
							 });
		}

		private static List<ProjectViewDTO> ToProjectViews(OptionalArrayOfRealmView realms)
		{
			var list = new List<ProjectViewDTO>();
			if (realms == null || !realms.HasValue || realms.Value == null)
			{
				return list;
			}

			foreach (var realm in realms.Value)
			{
				list.Add(new ProjectViewDTO
				{
					pid = realm.realmId,
					projectName = realm.displayName,
					parent = realm.parent.GetOrElse((string)null),
					children = new List<string>(realm.children.GetOrElse(Array.Empty<string>())),
					archived = realm.archived.GetOrElse(false),
					sharded = realm.sharded.GetOrElse(false),
				});
			}
