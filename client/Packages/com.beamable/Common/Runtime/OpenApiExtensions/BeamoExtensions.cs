...
namespace Beamable.Api.Autogenerated.Models
{
	public static class BeamoExtensions
	{
		...
		// --- BeamoBasic* overloads ---------------------------------------------------------------
		// The realm manifest (ManifestView) now carries the BeamoBasic* family; these mirror the
		// canonical overloads above. Basic fields are plain (bool/string) so the hash inputs read
		// directly; storage templateId stays an Optional (uses .Value like the canonical version).

		public static string ComputeChecksum(this BeamoBasicServiceReference reference)
		{
			return GetHash($"{reference.serviceName};{reference.imageId};{(reference.enabled ? "true" : "false")};{reference.templateId}");
		}

		public static void ResetChecksum(this BeamoBasicServiceReference reference)
		{
			reference.checksum = reference.ComputeChecksum();
		}

		public static string ComputeChecksum(this BeamoBasicServiceStorageReference reference)
		{
			return GetHash($"{reference.id};{reference.storageType};{(reference.enabled ? "true" : "false")};Some({reference.templateId.Value})");
		}

		public static void ResetChecksum(this BeamoBasicServiceStorageReference reference)
		{
			reference.checksum = reference.ComputeChecksum();
		}
