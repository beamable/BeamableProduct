...
namespace PubNubMessaging.Core
{
	...
	internal static class Helpers
	{
		...
		internal static TimetokenMetadata CreateTimetokenMetadata(object timeTokenDataObject, string whichTT)
		{
			...
			TimetokenMetadata timetokenMetadata = new TimetokenMetadata(
				Utility.CheckKeyAndParseLong(timeTokenData, whichTT, "t"),
		...
		internal static void AddToSubscribeMessageList(object dictObject, ref List<SubscribeMessage> subscribeMessages)
		{
			...
			if ((dict != null) && (dict.Count > 1))
			{
				...
				TimetokenMetadata originatingTimetoken = (dict.ContainsKey("o"))
					? CreateTimetokenMetadata(dict["o"], "Originating TT: ")
					: null;
				TimetokenMetadata publishMetadata =
					(dict.ContainsKey("p")) ? CreateTimetokenMetadata(dict["p"], "Publish TT: ") : null;
		                                                     ...
		                                                     Action<T> wildcardPresenceCallback)
		{
			...
			else
			{
				LoggingMethod.WriteToLog(
					string.Format(
						"DateTime {0}, CreateChannelEntity: channelOrChannelGroupName empty, is channel group {1}",
						DateTime.Now.ToString(), isChannelGroup.ToString()), LoggingMethod.LevelInfo);
		                                                           ...
		                                                           Action<T> wildcardPresenceCallback)
		{
			...
			else
			{
				LoggingMethod.WriteToLog(
					string.Format(
						"DateTime {0}, CreateChannelEntity 2: channelOrChannelGroupNames null, is channel group {1}",
						DateTime.Now.ToString(), isChannelGroup.ToString()), LoggingMethod.LevelInfo);
			}
		...
		internal static void LogChannelEntitiesDictionary()
		{
			...
			foreach (var ci in Subscription.Instance.ChannelEntitiesDictionary)
			{
				sbLogs.AppendFormat(
					...
					ci.Key.ChannelOrChannelGroupName, ci.Key.IsChannelGroup.ToString(),
					ci.Key.IsPresenceChannel.ToString()
				);
			}
		                                                             ...
		                                                             ref List<ChannelEntity> channelEntities)
		{
			...
			for (int index = 0; index < rawChannels.Length; index++)
			{
				...
				if (channelName.Length > 0)
				{
					...
					{
						channelIsSubscribed =
							Subscription.Instance.ChannelEntitiesDictionary[ce.ChannelID].IsSubscribed;
					}
		...
		internal static bool RemoveDuplicatesCheckAlreadySubscribedAndGetChannelsCommon<T>(ResponseType type,
			...
			Action<T> wildcardPresenceCallback,
			Action<T> disconnectCallback,
			string[] channelsOrChannelGroups,
			bool isChannelGroup,
			PubnubErrorFilter.Level errorLevel,
			bool unsubscribeCheck,
			ref List<ChannelEntity> channelEntities)
			...
			out List<ChannelEntity> channelEntities)
		{
			...
			{
				channelAdded = RemoveDuplicatesCheckAlreadySubscribedAndGetChannelsCommon<T>(
					type, userCallback, connectCallback, errorCallback,
					wildcardPresenceCallback, disconnectCallback, rawChannels, false, errorLevel, unsubscribeCheck,
					ref channelEntities
				);
			}
			...
			{
				channelGroupAdded = RemoveDuplicatesCheckAlreadySubscribedAndGetChannelsCommon<T>(
					type, userCallback, connectCallback, errorCallback,
					wildcardPresenceCallback, disconnectCallback, rawChannelGroups, true, errorLevel, unsubscribeCheck,
					ref channelEntities
				);
			}
		                                     ...
		                                     PubnubErrorFilter.Level errorLevel)
		{
			...
			object decodeMessage = (decryptMessage == "**DECRYPT ERROR**")
				? decryptMessage
				: jsonPluggableLibrary.DeserializeToObject(decryptMessage);
			...
			if (channelEntities != null && channelEntities.Count > 0)
			{
				foreach (ChannelEntity channelEntity in channelEntities)
				{
					...
					PubnubClientError error = Helpers.CreatePubnubClientError<T>(
						message, null, channelEntities, pnErrorCode,
		                                                    ...
		                                                    ref List<object> result)
		{
			try
			{
				...
				string multiChannelGroup =
					Helpers.GetNamesFromChannelEntities(pubnubRequestState.ChannelEntities, true);
			...
			if (pubnubRequestState.ChannelEntities != null)
			{
				...
				)
				{
					PubnubCallbacks.FireErrorCallbacksForAllChannels<T>(
						ex, pubnubRequestState, PubnubErrorSeverity.Critical,
		...
		{
			long timetoken = (subscribeMessage.PublishTimetokenMetadata != null)
				? subscribeMessage.PublishTimetokenMetadata.Timetoken
				: 0;
			...
			else
			{
				...
				if (cipherKey.Length > 0)
				{
					...
					try
					{
						...
						decodeMessage = (decryptMessage == "**DECRYPT ERROR**")
							? decryptMessage
							: jsonPluggableLibrary.DeserializeToObject(decryptMessage);
					}
			...
			if (asynchRequestState.ChannelEntities != null)
			{
				...
				for (int i = 0; i < asynchRequestState.ChannelEntities.Count; i++)
				{
					...
					if (channelEntity.ChannelParams.IsAwaitingConnectCallback)
					{
						...
						{
							case ResponseType.SubscribeV2:
								var connectResult = Helpers.CreateJsonResponse(
				...
				{
					Subscription.Instance.UpdateIsAwaitingConnectCallbacksOfEntity(
						asynchRequestState.ChannelEntities, false);
				}
			}
		}
		                                                             ...
		                                                             string channelGroups)
		{
			...
			LoggingMethod.WriteToLog(
				string.Format("DateTime {0}, PubnubClientError 2 = {1}", DateTime.Now.ToString(), error.ToString()),
				LoggingMethod.LevelInfo);
			return error;
		}
		                                                             ...
		                                                             string channelGroups)
		{
			...
			LoggingMethod.WriteToLog(
				string.Format("DateTime {0}, PubnubClientError 3 = {1}", DateTime.Now.ToString(), error.ToString()),
				LoggingMethod.LevelInfo);
			return error;
		}
		                                                             ...
		                                                             PubnubErrorSeverity severity)
		{
			...
			LoggingMethod.WriteToLog(
				string.Format("DateTime {0}, PubnubClientError 4 = {1}", DateTime.Now.ToString(), error.ToString()),
				LoggingMethod.LevelInfo);
			return error;
		}
