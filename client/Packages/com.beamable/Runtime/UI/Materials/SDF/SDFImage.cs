...
#pragma warning restore 649
		...
		private void GenerateSlicedMesh(VertexHelper vh)
		{
			...
			{
				startPosition, startPosition + new Vector2(borders.x, borders.y),
				endPosition - new Vector2(borders.z, borders.w), endPosition,
			};
			...
			{
				new Vector2(outer.x, outer.y), new Vector2(inner.x, inner.y), new Vector2(inner.z, inner.w),
				new Vector2(outer.z, outer.w),
			};

			var coordsValues = new Vector2[]
			{
				Vector2.zero, new Vector2(borders.x / size.x, borders.y / size.y),
				Vector2.one - new Vector2(borders.z / size.x, borders.w / size.y), Vector2.one,
			};
