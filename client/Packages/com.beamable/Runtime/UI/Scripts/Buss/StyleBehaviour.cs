...
namespace Beamable.UI.Buss
{
	public abstract class StyleBehaviour : UIBehaviour, IPointerEnterHandler, IPointerExitHandler
	{
		...
		public StyleBundle InlineStyleBundle =>
			_inlineStyleBundle ?? (_inlineStyleBundle = new StyleBundle
			{
		...
		public StyleObject DirectStyles
		{
			get;
			private set;
		}

		public StyleObject InheritedStyles
		{
			get;
			private set;
		}
		...
		public IEnumerable<StyleBehaviour> All()
		{
			...
			while (toExpand.Count > 0)
			{
				...
				if (visited.Contains(current))
					continue; // really, this could throw an error, because in a tree, we should never hit an element twice
