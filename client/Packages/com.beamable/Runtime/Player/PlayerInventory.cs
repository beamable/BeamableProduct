...
namespace Beamable.Player
{
	...
	public class PlayerInventory : IStorageHandler<PlayerInventory>, IServiceStorable
	{
		...
		{
			Debug.Log("Inventory updated! : " + string.Join(",", update.scopes));
			...
			foreach (var group in res.items)
			{
				...
				for (var i = 0; i < plrItems.Length; i++)
				{
					var item = plrItems[i] = new PlayerItem
					{
						...
						UniqueCode = (((group.items[i].id.GetHashCode() << 5) + group.items[i].id.GetHashCode()) ^ group.id.GetHashCode())
					};
				...
				_itemTrie.SetRange(group.id, plrItems);
			}
			
			Debug.Log("Trie updated...");
		}
