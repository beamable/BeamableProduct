...
namespace Beamable.Player
{
	...
	public class PlayerInventory : IStorageHandler<PlayerInventory>, IServiceStorable
	{
		...
		private async Promise<InventoryView> DownloadInventoryData(string[] scopes)
		{
			...
			var scopeTrie = new Trie<string>(); 
			foreach (var scope in scopes)
			{
				scopeTrie.Insert(scope, scope);
			}
			
			try
			{
				var res = await _openApi.ObjectPost(_userContext.UserId, req);
				return res;
			}
			catch (NoConnectivityException)
			{
				// TODO: maybe we just need to call Notify on the given scopes's groups, because the data will remain unchanged
				// if we can't download, then we need to simulate a fake `InventoryView` from the existing contents of the tries
				var itemScopes = scopeTrie.GetAll("items");
				var currencyScopes = scopeTrie.GetAll("currency");

				#region fake currencies from trie
				...
				{
					currencies = new CurrencyView[allCurrencies.Count],
					items = new ItemGroup[itemScopes.Count]
				};

				for (var i = 0; i < allCurrencies.Count; i++)
				{
					var props = new CurrencyProperty[allCurrencies[i]?.Properties.Count ?? 0];
					var propIndex = 0;
					var currencyProps = allCurrencies[i]?.Properties;
					if (currencyProps != null)
					{
						foreach (var kvp in currencyProps)
						{
							props[propIndex] = new CurrencyProperty {name = kvp.Key, value = kvp.Value};
							propIndex++;
						}
					}
					
					view.currencies[i] = new CurrencyView
					{
						id = allCurrencies[i].CurrencyId,
						amount = allCurrencies[i].Amount,
						properties = props
					};
				}
				#endregion
				
				#region fake items from trie
				// TODO: rewrite the items to use what actually exists...
				for (var i = 0; i < itemScopes.Count; i++)
				{
					...
					{
						id = itemScopes[i],
						items = new Beamable.Api.Autogenerated.Models.Item[items.Count]
					};
					for (var j = 0; j < items.Count; j++)
					{
						// TODO: pull out into local method
						var props = new Api.Autogenerated.Models.ItemProperty[items[j].Properties.Count];
						var propIndex = 0;
						foreach (var kvp in items[j].Properties)
						{
							props[propIndex] = new Api.Autogenerated.Models.ItemProperty
							{
						...
						view.items[i].items[j] = new Beamable.Api.Autogenerated.Models.Item
						{
							updatedAt = items[j].UpdatedAt,
							createdAt = items[j].CreatedAt,
							id = items[j].ItemId,
							properties = props
						};
					}
		...
		private async Promise GetLatestInventory()
		{
			try
			{
				...
				var res = await DownloadInventoryData(filteredScopes);
				...
				foreach (var group in res.items)
				{
					...
					var content = await contentRef.Resolve();
				...
				foreach (var currency in res.currencies)
				{
					...
					var content = await currRef.Resolve();
