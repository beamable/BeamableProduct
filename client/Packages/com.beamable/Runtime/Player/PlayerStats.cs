...
namespace Beamable.Player
{
	...
	[Serializable]
	public class SerializableDictionaryStringToPlayerStatCollection : SerializableDictionary<long, PlayerStatCollection>
	{
		
	}

	public class PlayerStatTypedCollection<TStat, TVal, TDict> 
	...
	[Serializable]
	public class PlayerStringStats : PlayerStatTypedCollection<PlayerStringStat, string, PlayerStringStatDictionary>
	{
		
	}
	
	public class PlayerStatCollection// : AbsObservableReadonlyDictionary<PlayerStat, SerializableDictionaryStringToPlayerStat>
	{
		private readonly IPlayerStatsFriend _statsFriend;

		public PlayerStringStats stringStats = new PlayerStringStats();
		...
		public PlayerStatCollection(IPlayerStatsFriend statsFriend, long playerId)
		{
	...
	public class PlayerStats2 : IPlayerStatsFriend
	{
		...
		public SerializableDictionaryStringToPlayerStatCollection playerIdToStats =
			new SerializableDictionaryStringToPlayerStatCollection();

		private HashSet<long> _playersToRefresh = new HashSet<long>();
		...
		public PlayerStatCollection MyStats => ForPlayer(_userContext.UserId);

		public PlayerStatCollection ForPlayer(long playerId)
		{
			if (!playerIdToStats.TryGetValue(playerId, out var stats))
			{
				playerIdToStats[playerId] = stats = new PlayerStatCollection(this, playerId);
				var _ = stats.Refresh();
			}
		...
		async Promise RequestPlayerUpdate(PlayerStatCollection collection)
		{
			_playersToRefresh.Add(collection.PlayerId);
			if (_syncPromise == null)
			{
				_syncPromise = LoadAllPlayers();
			}
		...
		async Promise LoadAllPlayers()
		{
			...
			var copy = new HashSet<long>(_playersToRefresh);

			using var sb = StringBuilderPool.StaticPool.Spawn();
			foreach (var playerId in copy)
			{
				sb.Builder.Append(playerId);
				sb.Builder.Append(",");
			}
			var query = sb.Builder.ToString();
			var result = await api.GetClientBatch(query, "stringlist");

			foreach (var kvp in result.playerIdToStats)
			{
				if (playerIdToStats.TryGetValue(kvp.Key, out var collection))
				{
