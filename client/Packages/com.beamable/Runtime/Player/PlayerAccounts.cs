...
namespace Beamable.Player
{
	...
	public class PlayerAccounts : AbsObservableReadonlyList<PlayerAccount>
	{
		...
		private readonly HashSet<int> _reportedRememberedTokenFailures = new HashSet<int>();
		...
		private static string GetRememberedTokenFailureReason(Exception ex)
		{
			if (ex is PlatformRequesterException platformRequesterException)
			{
				return platformRequesterException.Error?.error
				       ?? platformRequesterException.RequestError?.error
				       ?? ex.GetType().Name;
			}
		...
		private static long GetRememberedTokenFailureStatus(Exception ex)
		{
			return ex is PlatformRequesterException platformRequesterException
				? platformRequesterException.Status
				: 0;
		}

		private static int GetRememberedTokenDiagnosticKey(TokenResponse token)
		{
			unchecked
			{
				return ((token?.access_token?.GetHashCode() ?? 0) * 397)
				       ^ (token?.refresh_token?.GetHashCode() ?? 0);
			}
		}

		private static CoreEvent CreateRememberedTokenFailureEvent(long playerId, TokenResponse token,
			string reason, string action, string stage, long status)
		{
			var eventParams = new Dictionary<string, object>
			{
				...
				["action"] = action,
				["stage"] = stage,
				["access-token-last-4"] = TokenEvent.Last4(token?.access_token),
				["refresh-token-last-4"] = TokenEvent.Last4(token?.refresh_token)
			};
			if (status > 0)
			{
			...
			return new CoreEvent("user-tokens", "remembered_token_failure", eventParams);
		}

		private void ReportRememberedTokenFailure(RememberedAccountRefreshResult result)
		{
			if (result == null || string.IsNullOrEmpty(result.reason)) return;
			if (!_reportedRememberedTokenFailures.Add(GetRememberedTokenDiagnosticKey(result.token))) return;

			try
			{
				var status = result.status > 0 ? $" status=[{result.status}]" : "";
				BeamableLogger.LogWarning(
					$"[Beamable.Accounts.RememberedToken] reason=[{result.reason}] action=[{result.action}] stage=[{result.stage}]{status} " +
					$"cid=[{_ctx.Cid}] pid=[{_ctx.Pid}] access-token-last-4=[{TokenEvent.Last4(result.token?.access_token)}] " +
					$"refresh-token-last-4=[{TokenEvent.Last4(result.token?.refresh_token)}]");

				var settings = _provider.GetService<ITokenEventSettings>();
				if (!settings.EnableTokenAnalytics) return;
			...
			{
				// Diagnostics must never prevent Accounts from becoming ready.
			}
		}
		...
		private async Promise<RememberedAccountRefreshResult> TryRefreshRememberedAccount(TokenResponse token)
		{
			...
			{
				var terminal = IsTerminalRememberedTokenError(ex);
				return new RememberedAccountRefreshResult
				{
					token = token,
					tokenToRemove = terminal ? token : null,
					reason = GetRememberedTokenFailureReason(ex),
					action = terminal ? "removed" : "retained",
					stage = "get-user",
					status = GetRememberedTokenFailureStatus(ex),
					exception = ex
				};
			}
		}
		...
		protected override async Promise PerformRefresh()
		{
			...
			for (var i = 0; i < deviceTokens.Length; i++)
			{
				...
				{
					rememberedAccountResults.Add(new RememberedAccountRefreshResult
					{
						token = token,
			...
			for (var i = 0; i < rememberedAccountResults.Count; i++)
			{
				...
				ReportRememberedTokenFailure(result);
