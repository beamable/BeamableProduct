...
namespace Beamable.Api.Stats
{
	...
	{
		public StatsService(IPlatformService platform,
		                    IBeamableRequester requester,
		                    UserDataCache<Dictionary<string, string>>.FactoryFunction factoryFunction)
			: base(requester, platform, factoryFunction) { }

		protected override Promise<Dictionary<long, Dictionary<string, string>>> Resolve(
			string prefix,
			List<long> gamerTags)
		{
			string queryString = "";
			...
			return Requester.Request<BatchReadStatsResponse>(
			                ...
			                {
				                return OfflineCache
				                       .RecoverDictionary<string, string>(ex, "stats", Requester.AccessToken, gamerTags)
				                       .Map(
					                       stats =>
					                       {
						                       var results = stats.Select(kvp =>
						                       {
							                       return new BatchReadEntry
							                       {
								                       ...
								                       {
									                       k = statKvp.Key, v = statKvp.Value
								                       }).ToList()
							                       };
						                       }).ToList();

						                       var rsp = new BatchReadStatsResponse {results = results};
						                       return rsp;
					                       });
				                /*
