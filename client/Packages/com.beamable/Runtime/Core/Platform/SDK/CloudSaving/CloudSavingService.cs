...
namespace Beamable.Api.CloudSaving
{
	...
	public class CloudSavingService : PlatformSubscribable<ManifestResponse, ManifestResponse>
	{
		...
		private Action<Exception> ProvideErrorCallback(string methodName)
		{
			return (ex) =>
			{
				InvokeError($"{methodName} Failed: {ex?.Message ?? "unknown reason"}", ex);
			};
		}
			...
			GenerateUploadObjectRequestWithMapping()
		{
			...
			foreach (var fullPathToFile in allFilesAndDirectories)
			{
				...
				var lastModified =
					long.Parse(contentInfo.LastWriteTime.ToString("yyyyMMddHHmmss", CultureInfo.InvariantCulture));
		...
		private Promise<List<GetS3ObjectRequest>> GenerateDownloadRequest(
			ConcurrentDictionary<string, string> missingFiles)
		{
						...
						if (s3Response.TryGetValue(kv.Value, out s3PresignedURL))
						{
							promiseList.Add(
								new Func<Promise<Unit>>(
									() => GetOrPostObjectInS3(fullPathToFile, s3PresignedURL, method)));
						}
						else
						{
		                                   ...
		                                   UnityWebRequest request)
		{
			...
			for (var retryCount = 0; retryCount < 4; retryCount++)
			{
				...
				if (activeWebRequest.webRequest.IsHttpError())
				{
					...
					Debug.LogWarning(
						$"Retrying {currentRequest.method} for {filename}. Encountered: {currentRequest.error}");
