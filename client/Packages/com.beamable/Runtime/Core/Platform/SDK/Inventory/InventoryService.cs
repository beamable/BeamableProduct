...
namespace Beamable.Api.Inventory
{
	...
	public class InventoryService : AbsInventoryApi, IHasPlatformSubscriber<InventorySubscription, InventoryResponse, InventoryView>
	{
		...
		public InventoryService(IDependencyProvider provider, InventorySubscription subscription) : base(provider.GetService<IInventoryApi>(), provider.GetService<IUserContext>())
		{
			Subscribable = subscription;
		}

		public InventoryService(IDependencyProvider provider) : base(provider.GetService<IInventoryApi>(), provider.GetService<IUserContext>())
		{
			Subscribable = new InventorySubscription(provider);
		}

		[Obsolete("This constructor is deprecated and kept only for backward compatibility. Use InventoryService(IDependencyProvider provider) or InventoryService(IDependencyProvider provider, InventorySubscription subscription) instead.")]

		public InventoryService(IPlatformService platform, IBeamableRequester requester, IDependencyProvider provider) : base(provider.GetService<IInventoryApi>(), platform)
		{
			Subscribable = new InventorySubscription(provider);
		}
