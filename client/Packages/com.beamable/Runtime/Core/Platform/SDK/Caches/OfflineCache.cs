...
namespace Beamable.Api.Caches
{
	public class OfflineCache
	{
		...
		public static Promise<Dictionary<long, Dictionary<TKey, TValue>>> RecoverDictionary<TKey, TValue>(Exception ex,
			string key,
			IAccessToken token,
			List<long> gamerTags)
		{
		...
		private Promise<Dictionary<long, Dictionary<TKey, TValue>>> HandleDictionaryCase<TKey, TValue>(Exception ex,
			string key,
			List<long> gamerTags)
		{
			...
			else
			{
				if (!File.Exists(GetFullPathForKey(key)))
				{
					return Promise<T>.Failed(
						new NoConnectivityException(key + " is not cached and requires internet connectivity."));
				}
