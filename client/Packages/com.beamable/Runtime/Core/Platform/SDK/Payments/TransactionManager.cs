	...
	public class TransactionManager
	{
		...
		private void FulfillTransaction(CompletedTransaction transaction)
		{
			InAppPurchaseLogger.Log(
				$"[TransactionManager] FulfillTransaction: Fulfilling Transaction: {transaction.Txid}");
		...
		{
			InAppPurchaseLogger.Log(
				"[TransactionManager] BackupPendingPurchases: Saving pending purchases to disk, count: " +
				pendingPurchases.Count.ToString());
			...
			{
				InAppPurchaseLogger.Log(
					"[TransactionManager] BackupPendingPurchases: An error occurred during saving of pending purchases => " +
					e.Message);
			}
		}
		...
		{
			InAppPurchaseLogger.Log(
				"[TransactionManager] RestorePendingPurchases: Restoring unfulfilled transactions from disk...");
					...
					if (!string.IsNullOrEmpty(decodedPending))
					{
						...
						if (pending != null)
						{
							foreach (var entry in pending)
							{
								...
								InAppPurchaseLogger.Log(
									$"[TransactionManager] RestorePendingPurchases: Adding {txid} {listingSymbol} {entry.Key}");
			...
			{
				InAppPurchaseLogger.Log(
					"[TransactionManager] RestorePendingPurchases: An error occurred during loading pending purchases.");
		...
		{
			InAppPurchaseLogger.Log(
				$"[TransactionManager] BackupUnfulfilledTransactions: Saving Unfulfilled transactions to disk, count: {pendingFulfillment.Count}");

			try
			{
				var unfulfilledTransactionsList = new UnfulfilledTransactionList(pendingFulfillment);
				Dictionary<string, object> unfulfilledTransactions =
					JsonSerializable.Serialize(unfulfilledTransactionsList);
			...
			{
				InAppPurchaseLogger.Log(
					$"[TransactionManager] BackupUnfulfilledTransactions: An error occurred during saving of a transaction => {e.Message}");
			}
		}
		...
		{
			InAppPurchaseLogger.Log(
				"[TransactionManager] RestoreUnfulfilledTransactions: Restoring unfulfilled transactions from disk...");

			try
			{
				string encodedUnfulfilled = PlayerPrefs.GetString(unfulfilledStorageKey);
				InAppPurchaseLogger.Log(
					$"[TransactionManager] RestoreUnfulfilledTransactions: Decoding {encodedUnfulfilled}");
				if (!string.IsNullOrEmpty(encodedUnfulfilled))
				{
					...
					if (!string.IsNullOrEmpty(retrievedUnfulfilled))
					{
						...
						if (ts != null)
						{
							...
							{
								InAppPurchaseLogger.Log(
									$"[TransactionManager] RestoreUnfulfilledTransactions: Transactions restored, count: {transactions.UnfulfilledTransactions.Count}");
			...
			{
				InAppPurchaseLogger.Log(
					"[TransactionManager] RestoreUnfulfilledTransactions: An error occurred during loading Unfulfilled transactions.");
				Debug.LogException(e);
			}
