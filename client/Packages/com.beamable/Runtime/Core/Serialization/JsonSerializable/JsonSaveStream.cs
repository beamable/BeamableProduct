...
namespace Beamable.Serialization
{
	public partial class JsonSerializable
	{
		...
		public class JsonSaveStream : ClassPool<JsonSaveStream>, IStreamSerializer
		{
			...
			protected void AppendNumericValue(ulong number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(long number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(uint number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(int number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(ushort number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(short number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(byte number)
			{
				_builder.Append(number);
				AppendSeperator();
			}

			protected void AppendNumericValue(float number)
			{
				_builder.Append(number.ToString("R", System.Globalization.CultureInfo.InvariantCulture));
				AppendSeperator();
			}

			protected void AppendNumericValue(double number)
			{
				_builder.Append(number.ToString("R", System.Globalization.CultureInfo.InvariantCulture));
				AppendSeperator();
			}
			...
			public bool Serialize(string key, ref IDictionary<string, object> target)
			{
				...
				if (asTarget != null)
					...
					throw new NotSupportedException(
						"Serilization of non-Dictionary IDictionaries is not supported yet");
			}
