	...
	public static void KillAllWorkingProcessesForCommand(string commandLine)
	{
		...
		if (cmdProcesses.Length > 0)
		{
			for (int i = 0; i < cmdProcesses.Length; i++)
			{
				...
				{
					string commandLineOut = string.Empty;
					ProcessCommandLine.Retrieve(cmdProcesses[i], out commandLineOut);

					if (!string.IsNullOrEmpty(commandLineOut) && commandLineOut.Contains(commandLine))
					{
						cmdProcesses[i].Kill();
					}
				}
