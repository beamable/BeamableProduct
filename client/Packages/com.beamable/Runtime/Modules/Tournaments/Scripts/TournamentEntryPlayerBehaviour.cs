			...
			switch (State)
			{
				case TournamentLockState.LOW:
					...
					TargetTransform.anchoredPosition =
						new Vector2(content.anchoredPosition.x + LockLeftPadding, LockLowPadding);
					TargetTransform.sizeDelta =
						new Vector2(content.sizeDelta.x - (LockRightPadding + LockLeftPadding), TargetHeight);
					ActivateLowObjects();
					break;
				case TournamentLockState.TOP:
					...
					TargetTransform.anchoredPosition =
						new Vector2(content.anchoredPosition.x + LockLeftPadding, -LockTopPadding);
					TargetTransform.sizeDelta =
						new Vector2(content.sizeDelta.x - (LockRightPadding + LockLeftPadding), TargetHeight);
					ActivateTopObjects();
					break;
