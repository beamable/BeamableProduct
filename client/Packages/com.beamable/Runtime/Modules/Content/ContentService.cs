...
namespace Beamable.Content
{
    ...
    public class ManifestSubscription : PlatformSubscribable<ClientManifest, ClientManifest>
    {
        ...
        public ManifestSubscription(IDependencyProvider provider,
                                    string manifestID, ClientManifest bakedManifest, bool omitTags = false) : base(provider, "content")        {
            _provider = provider;
        ...
        private bool TryGetCachedManifest(string uid, out ClientManifest manifest)
        {
            ...
            try
            {
	            if (_bakedManifest.uid.TryGet(out var bakedUid) && uid.Equals(bakedUid))
	            {
		            manifest = _bakedManifest;
		            Debug.Log($"Returning cached manifest for uid={uid} from baked manifest");
        ...
        private void SaveCachedManifest(string uid, ClientManifest manifest)
        {
            try
            {
                ...
                Debug.Log($"Saving cached manifest for uid={uid} to {path}");
...
#pragma warning restore 0809
        ...
        protected override Promise<ClientManifest> ExecuteRequest(IBeamableRequester requester, string url)
        {
            ...
            return _contentApi.GetManifestChecksum(ManifestID).FlatMap(checksumResponse =>
            {
	            ...
	            return _provider.GetService<IManifestResolver>()
        ...
        IHasPlatformSubscribers<ManifestSubscription, ClientManifest, ClientManifest>
    {
                              ...
                              IBeamableFilesystemAccessor filesystemAccessor, ContentParameterProvider config, OfflineCache offlineCache)
        {
            ...
            Subscribable = new ManifestSubscription(_provider, CurrentDefaultManifestID, BakedManifest, _config.OmitContentManifestTags);            Subscribable.Subscribe(cb =>
            {
        ...
        private void AddSubscriber(string manifestID)
        {
            ...
            Subscribables.Add(manifestID, new ManifestSubscription(_provider, manifestID, BakedManifest, _config.OmitContentManifestTags));            Subscribables[manifestID].Subscribe(cb => { });
        }
