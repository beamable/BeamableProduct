...
namespace Beamable.Shop
{
	...
	public class ShopFlow : MonoBehaviour
	{
		...
		private Promise<List<PlatformSubscription<PlayerStoreView>>> _subscriptionsPromise =
			new Promise<List<PlatformSubscription<PlayerStoreView>>>();
		...
		void RenderShop(StoreRef store)
		{
			...
			if (_storeData.TryGetValue(store, out var storeData))
			{
				...
				MenuManager.Show<BasicStoreRenderer>(menu =>
				{
		...
		{
			MenuManager.Show<ShopRewardRenderer>(menu =>
			{
				menu.Listing = data;
			});
		}
