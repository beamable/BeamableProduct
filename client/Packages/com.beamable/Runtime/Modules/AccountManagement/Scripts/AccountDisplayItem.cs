...
namespace Beamable.AccountManagement
{
	public class AccountDisplayItem : MonoBehaviour
	{
		...
		public GameObject ThirdPartiesGroup;
		...
		public Promise<List<AccountManagementConfiguration.UserThirdPartyAssociation>> ThirdPartyAssociationPromise
		{
			get;
			private set;
		}
		...
		public void Apply()
		{
			...
			LoadingPromise.Then(_ =>
			{
				...
				{
					bool hasDBCredentials = User.HasDBCredentials();
					bool hasApple = false;
					bool hasFb = false;

					EmailIcon.SetActive(hasDBCredentials);
					...
					if (ThirdPartiesGroup != null)
					{
						ThirdPartiesGroup.SetActive(hasDBCredentials || hasFb || hasApple);
					}
