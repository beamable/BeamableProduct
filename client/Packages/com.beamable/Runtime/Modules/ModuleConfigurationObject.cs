...
namespace Beamable
{
	...
	{
		private static Dictionary<Type, BaseModuleConfigurationObject> _typeToConfig =
			new Dictionary<Type, BaseModuleConfigurationObject>();
		...
		public static TConfig Get<TConfig>() where TConfig : BaseModuleConfigurationObject
		{
			...
			if (data == null)
			{
				...
				UnityEditor.AssetDatabase.ImportAsset(
					assetPath, UnityEditor.ImportAssetOptions.DontDownloadFromCacheServer);
