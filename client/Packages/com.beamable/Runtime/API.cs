...
namespace Beamable
{
	...
	public interface IBeamableAPI
	{
		...
		CloudSavingService CloudSavingService
		{
			get;
		}
		...
		Promise<IBeamablePurchaser> BeamableIAP
		{
			get;
		}
		...
		ITournamentApi TournamentsService
		{
			get;
		}

		ICloudDataApi TrialDataService
		{
			get;
		}

		[Obsolete("Use " + nameof(TournamentsService) + " instead.")]
		ITournamentApi Tournaments
		{
			get;
		}
		...
		private Promise<IBeamableAPI> Initialize()
		{
			if (Application.isPlaying)
			{
				...
				switch (promiseHandlerConfig)
				{
					...
					{
						PromiseExtensions.RegisterBeamableDefaultUncaughtPromiseHandler(
							promiseHandlerConfig == CoreConfiguration.EventHandlerConfig.Replace);
						break;
					}
					default:
