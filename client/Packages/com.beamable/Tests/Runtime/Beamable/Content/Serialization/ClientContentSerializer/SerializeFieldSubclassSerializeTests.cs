    ...
    ""spriteAssetName"": {
     ...
     ""bigIcons"": {
      ...
      ""amountFormat"": {
         ...
         Assert.AreEqual("spendable_tokens", typeof(GFCurrencyContent).GetField("spriteAssetName", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(o));
         Assert.AreEqual("tuna", typeof(GFCurrencyContent).GetField("amountFormat", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(o));
         Assert.IsTrue( (typeof(GFCurrencyContent).GetField("currencyTags", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(o) as string[]).Length == 1);
      }
      ...
      public void PrivateFieldsInParentClassesAreSerialized()
      {
         ...
         typeof(GFCurrencyContent).GetField("spriteAssetName", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(c, "spendable_tokens");
         typeof(GFCurrencyContent).GetField("amountFormat", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(c, "tuna");
         typeof(GFCurrencyContent).GetField("currencyTags", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(c, new []{"shop"});
...
#pragma warning disable 0649
      ...
      [SerializeField, Tooltip("Parameter 0 is icon, parameter 1 is amount, default: '{0} {1}'")] string amountFormat;
