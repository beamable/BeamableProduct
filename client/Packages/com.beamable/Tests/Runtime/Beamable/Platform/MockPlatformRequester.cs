...
namespace Beamable.Platform.Tests
{
	public class MockPlatformRequester : IBeamableRequester
	{
		...
		private Dictionary<Type, RequestJsonFunction<object>> _jsonFunctions =
			new Dictionary<Type, RequestJsonFunction<object>>();
		...
		{
			_jsonFunctions.Add(
				typeof(T), (method, uri, body, header) => function(method, uri, body, header).Map(o => (object)o));
		}
