...
namespace Beamable.Tests.Modules.Shop.ListingContentTests
{
	public class ValidateListingTests
	{
		...
		public void StoreWithNoTitles_YieldsNoValidationErrors()
		{
			...
			listing.offer = new ListingOffer {titles = new OptionalNonBlankStringList {HasValue = false}};
		...
		public void ActivePeriod_WithNone_WithInvalidDate_YieldsNoValidationErrors()
		{
			...
			{
				HasValue = false,
				Value = new ActivePeriod {end = new OptionalString {HasValue = false}, start = "not-a-date"}
			};
		...
		public void ActivePeriod_WithSome_WithInvalidDate_YieldsValidationError()
		{
			...
			Assert.AreEqual(
				"Error with 'System.String start' on Beamable.Common.Shop.ActivePeriod. date is not in 8601 iso format. yyyy-MM-ddTHH:mm:ssZ",
				errors[0]);
		}
	}
}
