...
namespace Beamable.Editor.Login.UI.Components
{
	public class AccountSummaryVisualElement : LoginBaseComponent
	{
		...
		public override void Refresh()
		{
			...
			_emailField.RegisterValueChangedCallback(
				evt => _emailField.SetValueWithoutNotify(Model?.CurrentUser?.email));

			_aliasField = Root.Q<TextField>("alias");
			_aliasField.RegisterValueChangedCallback(
				evt => _aliasField.SetValueWithoutNotify(Model?.Customer?.CidOrAlias));

			_roleField = Root.Q<TextField>("role");
			_roleField.RegisterValueChangedCallback(
				evt => _roleField.SetValueWithoutNotify(Model?.CurrentUser?.roleString));

			_gameField = Root.Q<TextField>("game");
			_gameField.RegisterValueChangedCallback(
				evt => _gameField.SetValueWithoutNotify(Model?.CurrentGame?.ProjectName));

			_realmField = Root.Q<TextField>("realm");
			_realmField.RegisterValueChangedCallback(
				evt => _realmField.SetValueWithoutNotify(Model?.CurrentRealm?.ProjectName));
