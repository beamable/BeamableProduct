...
namespace Beamable.Editor.Login.UI.Components
{
	public class NewUserVisualElement : LoginBaseComponent
	{
		...
		{
			return
				"Create a new account. You will need to check with your Organization's administrator to receive full access to the project.";
		}
		...
		public override void Refresh()
		{
			...
			var isPasswordValid =
				_passwordTextField.AddErrorLabel("Password", PrimaryButtonVisualElement.PasswordErrorHandler);
			...
			var doPasswordsMatch = _passwordConfirmTextField.AddErrorLabel(
				"Password Match", m => m != _passwordTextField.value
			...
			_legalButton.OnClick += () =>
			{
				Application.OpenURL(BeamableConstants.BEAMABLE_LEGAL_WEBSITE);
			};
