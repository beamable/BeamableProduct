...
namespace Beamable.Editor.Login.UI
{
	public class LoginErrorHandlers
	{
		...
		public string ProduceError(Exception ex)
		{
			...
			if (ex is PlatformRequesterException err && _predicateToErrorGenerator.TryGetValue(
				_predicateToErrorGenerator.Keys.FirstOrDefault(p => p(err)) ?? (_ => false), out var errorMessage))
			{
