...
namespace Beamable.Editor.Content.Models
{
	public class ContentDataModel
	{
		...
		public EditorContentQuery Filter
		{
			get;
			private set;
		}

		public EditorContentQuery SystemFilter
		{
			get;
			private set;
		} = new EditorContentQuery();
		...
		private Dictionary<string, ManifestReferenceSuperset> _lastServerManifest =
			new Dictionary<string, ManifestReferenceSuperset>();
		...
		private Dictionary<string, ContentItemDescriptor>
			_idToContent = new Dictionary<string, ContentItemDescriptor>();

		private Dictionary<string, ContentTypeDescriptor> _nameToType = new Dictionary<string, ContentTypeDescriptor>();

		private Dictionary<string, ContentTagDescriptor> _tagToDescriptor =
			new Dictionary<string, ContentTagDescriptor>();
		...
		public List<ContentTypePair> ContentTypePairs
		{
			get;
			private set;
		}

		public LocalContentManifest LocalManifest
		{
			get;
			private set;
		}
		...
		public void ContentItemRename(ContentItemDescriptor contentItemDescriptor)
		{
			if (_content.IndexOf(contentItemDescriptor) == -1)
			{
				BeamableLogger.LogError(
					new Exception("ContentItemDescriptor_OnModified() error. Item must exist in list."));
				return;
			}
			...
			{
				BeamableLogger.LogError(
					new Exception("ContentItemDescriptor_OnModified() error. contentObject must be not null."));
				return;
			}
		...
		public void ToggleValidationFilter(ContentValidationStatus status, bool shouldFilterOn)
		{
			var statusExistsInFilter =
				(Filter?.HasValidationConstraint ?? false) && Filter.ValidationConstraint == status;
