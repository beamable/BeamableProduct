			...
			try
			{
				var provider = new SettingsProvider($"Project/Beamable", SettingsScope.Project)
				{
					activateHandler = (searchContext, rootElement) =>
					{
						...
						{
							ConfigManager
								.Initialize(); // re-initialize every time the window is activated, so that we make sure the SO's always exist.
							...
							if (ConfigManager.MissingAnyConfiguration)
							{
								...
								{
									text =
										$"Welcome to Beamable! These configurations need to be created:\n{missingConfigs}"
								};
