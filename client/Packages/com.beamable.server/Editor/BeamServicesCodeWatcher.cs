...
namespace Beamable.Server.Editor
{
	...
	public class BeamServicesCodeWatcher : IBeamHintSystem
	{
		...
		public static void GenerateClientSourceCodeFromOpenApi(MicroserviceDescriptor service)
		{
			...
			de.Requester.Request(Method.GET, endpoint, null, true, s => s).Then(s =>
			{
				...
				var generatorDesc = GetGeneratorDescriptor(service);
				generatorDesc.CustomClientPath = service.CustomClientPath;
				if (!Directory.Exists(service.AutoGeneratedClientPath))
				{
					Directory.CreateDirectory(service.AutoGeneratedClientPath);
				}
				var clientPath = Path.Combine(service.AutoGeneratedClientPath, $"{apiCodeGenerator.Descriptor.Name}Client.cs");
				if (File.Exists(clientPath))
				{
					File.Delete(clientPath);
				}
				File.WriteAllText(clientPath, code);
