...
namespace Beamable.Server.Editor
{
	...
	public class BeamServicesCodeWatcher : IBeamHintSystem
	{
		...
		private static void GenerateClientSourceCode(MicroserviceDescriptor service)
		{
			...
			var buildCommand = new BuildImageCommand(service, false, true, "_generator");
			buildCommand.Start(null).Then(_ =>
			{
				var check = new CheckImageReturnableCommand(service.ClientGeneratorContainerName);
				check.Start(null).Then(isRunning =>
				{
					if (isRunning) return;

					Debug.Log("--!!!--!!!- STARTING!!");
					var clientCommand = new RunClientGenerationCommand(service);
					clientCommand.Start();
				});
			});
