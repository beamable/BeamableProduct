...
namespace Beamable.Editor.UI.Model
{
	...
	public class MicroserviceModel : ServiceModelBase, IBeamableMicroservice
	{
		...
		public MicroserviceBuilder ServiceBuilder
		{
			get;
			protected set;
		}
		...
		public List<MongoStorageModel> Dependencies
		{
			get;
			private set;
		} = new List<MongoStorageModel>();

		public override bool IsRunning => ServiceBuilder?.IsRunning ?? false;
		public bool IsBuilding => ServiceBuilder?.IsBuilding ?? false;

		public bool SameImageOnRemoteAndLocally =>
			string.Equals(ServiceBuilder?.LastBuildImageId, RemoteReference?.imageId);
		...
		public override void PopulateMoreDropdown(ContextualMenuPopulateEvent evt)
		{
			...
			evt.menu.BeamableAppendAction($"Visual Studio Code/Copy Debug Configuration{debugToolsSuffix}", pos =>
			{
				CopyVSCodeDebugTool();
			}, IncludeDebugTools);
