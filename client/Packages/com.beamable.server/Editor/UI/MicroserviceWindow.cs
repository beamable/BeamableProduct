...
namespace Beamable.Editor.Microservice.UI
{
	public class MicroserviceWindow : CommandRunnerWindow, ISerializationCallbackReceiver
	{
		...
		void SetForContent()
		{
			...
			_microserviceBreadcrumbsVisualElement =
				root.Q<MicroserviceBreadcrumbsVisualElement>("microserviceBreadcrumbsVisualElement");
			_microserviceBreadcrumbsVisualElement.Refresh();
			_microserviceBreadcrumbsVisualElement.SetSelectAllCheckboxValue(
				Model?.Services?.Count > 0 && Model.Services.All(model => model.IsSelected));
			...
			_microserviceContentVisualElement =
				root.Q<MicroserviceContentVisualElement>("microserviceContentVisualElement");
