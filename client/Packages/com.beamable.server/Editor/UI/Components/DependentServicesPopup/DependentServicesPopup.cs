...
namespace Beamable.Editor.Microservice.UI.Components
{
	...
	{
		public Dictionary<MicroserviceModel, DependentServicesMicroserviceEntryVisualElement> MicroserviceEntries
		{
			get;
			private set;
		}

		public Dictionary<MongoStorageModel, DependentServicesStorageObjectEntryVisualElement> StorageObjectEntries
		{
			get;
			private set;
		}

		public bool IsAnyRelationChanged
		{
			get;
			private set;
		} = false;
		...
		private readonly Dictionary<MicroserviceModel, List<ServiceRelationInfo>> _changedDependencies =
			new Dictionary<MicroserviceModel, List<ServiceRelationInfo>>();
		...
		{
			StorageObjectEntries =
				new Dictionary<MongoStorageModel, DependentServicesStorageObjectEntryVisualElement>(
		...
		{
			MicroserviceEntries =
				new Dictionary<MicroserviceModel, DependentServicesMicroserviceEntryVisualElement>(
					MicroservicesDataModel.Instance.Services.Count);
		...
		private void ChangeSelectionHighlight(bool state)
		{
			...
			{
				checkboxVisualElement =
					entry.DependentServices.FirstOrDefault(
						x => x.MongoStorageModel == _lastRelationChangedStorageObject);
				checkboxVisualElement?.EnableInClassList("sectionHighlight", state);
				;
			}

			checkboxVisualElement =
				_emptyRowFillEntry.DependentServices.FirstOrDefault(
					x => x.MongoStorageModel == _lastRelationChangedStorageObject);
			checkboxVisualElement?.EnableInClassList("sectionHighlight", state);
			;
		}
		...
		private IEnumerable<MongoStorageModel> GetDependentServices(MicroserviceModel microserviceModel)
		{
			...
			{
				if (microserviceAssemblyDefinition.References.Contains(
					AssemblyDefinitionHelper.ConvertToInfo(storageObjectAssemblyDefinitionsAsset.Value).Name))
				{
