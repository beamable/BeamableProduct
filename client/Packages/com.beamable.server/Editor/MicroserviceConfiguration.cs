...
namespace Beamable.Server.Editor
{
	...
	public class MicroserviceConfiguration : AbsModuleConfigurationObject<MicroserviceConfigConstants>
	{
      ...
      const string DOCKER_LOCATION = "/usr/local/bin/docker";
		...
		[Tooltip(
			 "When you run a microservice in the Editor, the prefix controls the flow of traffic. By default, the prefix is your MAC address. If two developers use the same prefix, their microservices will share traffic. The prefix is ignored for games running outside of the Editor."),
		 Delayed]
		...
		[Tooltip(
			"When you build a microservice, any ContentType class will automatically be referenced if this field is set to true. Beamable recommends that you put your ContentTypes into a shared assembly definition instead.")]
		...
		[Tooltip(
			"Docker Buildkit may speed up and increase performance on your microservice builds. However, it is not fully supported with Beamable microservices, and you may encounter issues using it. ")]
		...
		public string ValidatedDockerCommand =>
			string.IsNullOrWhiteSpace(DockerCommand) ? DOCKER_LOCATION : DockerCommand;

		private bool _enableStoragePreviewCached = false;

#if !BEAMABLE_LEGACY_MSW
		[Tooltip(
			"Microservice Logs are sent to a dedicated logging window. If you enable this field, then service logs will also be sent to the Unity Console.")]
	...
	public class MicroserviceConfigurationEntry
	{
		...
		[Tooltip(
			"When building locally, should the service be build with debugging tools? If false, you cannot attach breakpoints.")]
	...
	public class MicroserviceConfigurationDebugEntry
	{
		...
		[Tooltip(
			"The SSH password to use to connect a debugger. This is only supported for local development. SSH is completely disabled on cloud services.")]
