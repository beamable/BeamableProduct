...
namespace Beamable.Server.Editor.Usam
{
	public class CodeService : ILoadWithContext
	{
		...
		public async Promise GenerateClientCode(string id)
		{
			...
			var microservicePath = $"{StandaloneMicroservicesPath}{id}/";
			var beamPath = BeamCliUtil.CLI_ACTUAL_PATH;
			var buildCommand = $"build {microservicePath} '/p:BeamableTool={beamPath}'";
			
			LogVerbose($"Starting build service: {id}");
			await _dotnetService.Run(buildCommand);
			
			LogVerbose($"Starting beam client code generator");

			string dllPath = $"{microservicePath}services/{id}/bin/Debug/net6.0/{id}.dll";
