		...
		, ILoadWithContext
	{
		                                              ...
		                                              List<BeamFederationEntry> federations)
		{
			var args = new FederationSetArgs
			{
				...
				fedId = new string[federations.Count],
				fedType = new string[federations.Count]
			};
			for (var i = 0; i < federations.Count; i++)
			{
				args.fedId[i] = federations[i].federationId;
				args.fedType[i] = federations[i].interfaceName;
			}

			if (args.fedType.Length == 0)
			{
				args.clear = true;
			}
