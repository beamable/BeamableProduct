...
namespace Beamable.Server.Editor.Uploader
{
	public class ContainerUploader
	{
								 ...
								 string imageId)
		{
			...
			var serviceUniqueName = GetHash($"{api.CustomerView.Cid}_{api.ProductionRealm.Pid}_{descriptor.Name}")
				.Substring(0, 30);
		...
		private async Task<Dictionary<string, object>> UploadLayer(string layerPath)
		{
			...
			{
				{"digest", layerDigest.Digest}, {"size", layerDigest.Size}, {"mediaType", MediaLayer}
			};
		}
		...
		private async Task<Uri> PrepareUploadLocation()
		{
			...
			catch (HttpRequestException ex)
			{
				var body = await response.Content.ReadAsStringAsync();
				Debug.LogError(
					$"Failed to prepare image upload location. message=[{ex.Message}] body=[{body}] url=[{uri}]");
				throw ex;
			}
