...
namespace Beamable.Server
{
	...
	{
		private Dictionary<string, AssemblyDefinitionInfo> _assemblies =
			new Dictionary<string, AssemblyDefinitionInfo>();
	...
	public class MicroserviceAssemblyDependency
	{
		...
		public string DockerBuildLocation =>
			$"/src/{MicroserviceDescriptor.ASSEMBLY_FOLDER_NAME}{Assembly.Location}"; // lib/path
	}
	...
	public class DependencyResolver : MonoBehaviour
	{
		...
		private static bool IsStubbedType(Type t)
		{
			var stubbed = new Type[]
			{
				...
				typeof(ArrayDict), typeof(JsonSerializable.IStreamSerializer), typeof(ContentObject),
				typeof(IContentRef), typeof(ContentDelegate)
			};
									  ...
									  string assemblyName)
		{
			...
			{
				return
					true; // this assembly is okay... We couldn't search for it above because the user may not even have it installed :/
			}
			...
			while (unityAssembliesToExpand.Count > 0)
			{
				...
				if (!allRequiredUnityAssemblies.Contains(curr))
				{
					...
					foreach (var referenceName in curr.References)
					{
						...
						{
							Debug.LogWarning(
								$"Skipping {referenceName} because it is a stubbed package. You should still install the package for general safety.");
						}
					}
				}
			}
		...
		private static List<MicroserviceFileDependency> GatherSingleFileDependencies(
