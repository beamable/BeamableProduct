...
namespace Beamable.Server.Editor.DockerCommands
{
	...
	public class RunClientGenerationCommand : RunImageCommand
	{
		...
		: base(service.ImageName + "_generator", service.ClientGeneratorContainerName, service)
		{
			//docker run --rm -it --mount 'type=bind,source="/Users/chris/Documents/GitHub/BeamableProduct/client/Assets/Beamable/Autogenerated/Microservices",dst=/client-output' --mount 'type=bind,source="/Users/chris/Documents/GitHub/BeamableProduct/client/Temp/beam/Simple",dst=/subapp/simple' --entrypoint dotnet --name Simple_container_clientGeneration simple run -- generate-client /client-output
			//docker run --rm -it --mount 'type=bind,source=/Users/chris/Documents/GitHub/BeamableProduct/client/Temp/beam/Simple,dst=/subapp/simple' --mount 'type=bind,source=/Users/chris/Documents/GitHub/BeamableProduct/client/Assets/Beamable/AutoGenerated/Microservices,dst=/client-output' --name xyzw --entrypoint dotnet simple run -- generate-client /client-output
			_service = service;
		}
		...
		protected override string GetCustomDockerFlags()
		{
			...
			var sourceMount = $"--mount 'readonly,type=bind,source=\"{fullBuildPath}\",dst=/subapp/{_service.ImageName}'";

			var clientPath = Path.GetFullPath($"Assets/Beamable/Autogenerated/Microservices");
			var clientMount = $"--mount 'type=bind,source=\"{clientPath}\",dst=/client-output'";

			var rawSourcePath =Path.GetDirectoryName(Path.GetFullPath(_service.AttributePath));
			var asmName = _service.ConvertToInfo().Name;
			var rawSourceMount =
				$"--mount 'readonly,type=bind,source=\"{rawSourcePath}\",dst=/subapp/{_service.ImageName}/{asmName}'";

			return $"{clientMount} {sourceMount} {rawSourceMount} --entrypoint dotnet";
		}
