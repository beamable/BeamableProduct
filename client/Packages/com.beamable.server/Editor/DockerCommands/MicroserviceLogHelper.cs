...
namespace Beamable.Server.Editor.DockerCommands
{
	public static class MicroserviceLogHelper
	{
		...
		{
			LogConstants.STARTING_PREFIX, LogConstants.SCANNING_CLIENT_PREFIX,
			LogConstants.REGISTERING_STANDARD_SERVICES, LogConstants.REGISTERING_CUSTOM_SERVICES,
			LogConstants.SERVICE_PROVIDER_INITIALIZED, LogConstants.EVENT_PROVIDER_INITIALIZED
		};
...
#pragma warning restore 219
				...
				{
					Debug.Log(
						$"{WithColor(Color.grey, $"[{label}]")} {WithColor(color, $"[{logLevel}]")} {WithColor(darkColor, $"{message}\n{objsToString}")}");
				}
#else
