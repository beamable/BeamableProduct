...
namespace Beamable.Server
{
	public class ECRUploaderService
	{
									...
									string registryUrl="https://dev-ecr.beamable.com")
		{
			...
			_client.DefaultRequestHeaders.Add("x-beam-clientid", cid);
			_client.DefaultRequestHeaders.Add("x-beam-projectid", pid);
			_client.DefaultRequestHeaders.Add("x-beam-token", token);
		...
		private async Task<HttpResponseMessage> SendFileToS3(Uri uri, ObservableByteArrayContent content, CancellationToken cancellationToken)
		{
		...
		private ObservableByteArrayContent GenerateContentForUpload(string fileName)
		{
			GenerateContentForUpload(fileName, out var content, out var length);
			return content;
		}

		private void GenerateContentForUpload(string fileName, out ObservableByteArrayContent content, out long length)
		{
			var file = ReadFileWithoutLock(fileName);

			length = file.Length;
			content = new ObservableByteArrayContent(file);
		}
		...
		public async Task Upload(string folder, CancellationToken token)
		{
			...
			var uploadByteLayers = new ObservableByteArrayContent[manifest.layers.Length];
			long totalByteSize = 0;
			for (var i = 0; i < manifest.layers.Length; i++)
			{
				var layer = manifest.layers[i];
				GenerateContentForUpload($"{folder}/{layer}", out uploadByteLayers[i], out var length);
				totalByteSize += length;
			}
			...
			for (var i = 0; i < manifest.layers.Length; i++)
			{
				...
				var bytes = uploadByteLayers[i];
				bytes.OnProgress += (position, progress) =>
				{
					Emit();
				};
				var work = SendFileToS3(url, bytes, token);
			...
			void Emit()
			{
				long currentCompletedLength = 0;
				foreach (var kvp in uploadIndexToJob)
				{
					currentCompletedLength += uploadByteLayers[kvp.Key].Position;
				}
				var progress = currentCompletedLength / (float)totalByteSize;
				EmitProgress(.1f + (progress * .83f));
			}
	...
	public class ObservableByteArrayContent : ByteArrayContent
	{
		...
		public ObservableByteArrayContent(byte[] content) : base(content)
		{
			_bytes = content;
		...
		protected override async Task SerializeToStreamAsync(Stream stream, TransportContext context)
		{
			...
			{
				await stream.WriteAsync(_bytes, i, Math.Min(chunkSize, _bytes.Length - i));
				_position = i;
				Progress = i / _length;
