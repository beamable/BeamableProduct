...
/project
  ...
  /services
     /Timeout
        ...
        Timeout.cs
     /TimeoutLibrary
        TimeoutLibrary.csproj
        TimeoutLibrary.cs
```

## Running Locally
The Timeout project can be run locally by starting it from an IDE, or by executing the `dotnet run` command.
...
```shell
beam services deploy --ids Timeout
```

### MSBuild Properties
The `Timeout.csproj` file has a few configuration options that can be set.
```xml
<PropertyGroup>
...
#### BeamableTool
By default, both Timeout and TimeoutLibrary have the BeamCLI installed as a project tool. 
...
remote swagger. 
```shell
beam project open-swagger Timeout
```
...
at each linked Unity project's `Assets/Beamable/Microservice/Microservices/TimeoutClient.cs`. 

Additionally, the `share-code` msbuild target in the TimeoutLibrary project will copy their
