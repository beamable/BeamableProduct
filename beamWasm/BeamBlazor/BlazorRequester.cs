...
public class BlazorRequester : IBeamableRequester
{
	string Host => _appContext.Host;
	private readonly IAppContext _appContext;
	public IAccessToken AccessToken => _appContext.Token;
	public string Cid  => _appContext.Cid;
	public string Pid  => _appContext.Pid;

	public BlazorRequester(IAppContext appContext)
	{
	...
	private HttpClient GetClient(bool includeAuthHeader, string pid, string cid, IAccessToken? token, bool customerScoped)
	{
		...
		// _httpClient.DefaultRequestHeaders.Add("contentType", "application/json"); // confirm that it is required
		httpClient.DefaultRequestHeaders.Add("X-DE-SCOPE", customerScoped ? cid : $"{cid}.{pid}");
	...
	{
		_appContext.UpdateToken(tokenResponse);
		return this;
	}
