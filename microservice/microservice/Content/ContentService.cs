...
namespace Beamable.Server.Content
{
	...
	{
		public UnreliableContentService(MicroserviceRequester requester, SocketRequesterContext socket,
			IContentResolver contentResolver) : base(requester, socket, contentResolver)
		{
		}
	...
	public class ContentService : IMicroserviceContentApi
	{
		...
		private readonly ConcurrentDictionary<string, ContentReference> _idToContentReference =
			new ConcurrentDictionary<string, ContentReference>();
					...
					.Map(manifest =>
					{
						...
						foreach (var reference in manifest.references)
						{
							...
							if (!_idToContentReference.ContainsKey(reference.id))
							{
								...
								{
									BeamableLogger.LogWarning(
										"Couldn't add content id=[{id}] because it already existed.", reference.id);
								}
							}
						}
		...
		public Promise<TContent> Resolve<TContent>(IContentRef<TContent> reference)
