...
namespace Beamable.Server
{
	...
	public class BeamableMicroService
	{
		...
		public string QualifiedName =>
			$"{_args.NamePrefix}micro_{MicroserviceName}"; // scope everything behind a feature name "micro"

		public string PublicHost =>
			$"{_args.Host.Replace("wss://", "https://").Replace("/socket", "")}/basic/{_args.CustomerID}.{_args.ProjectName}.{QualifiedName}/";
		...
		private int
			_refuseNewClientMessageFlag =
				0; // https://stackoverflow.com/questions/29411961/c-sharp-and-thread-safety-of-a-bool
		...
		// default is false, set 1 for true.
		private int
			_shutdownStarted = 0; // https://stackoverflow.com/questions/29411961/c-sharp-and-thread-safety-of-a-bool
		...
		private int[] _retryIntervalsInSeconds = new[] { 5, 5, 10, 10, 15, 45, 60 };
			...
			where TMicroService : Microservice
		{
			...
			Log.Debug(
				...
				" {host} {prefix} {cid} {pid} {sdkVersionExecution} {sdkVersionBuild} {disableCustomHooks}", args.Host,
				args.NamePrefix, args.CustomerID, args.ProjectName, args.SdkVersionExecution, args.SdkVersionBaseBuild,
				args.DisableCustomInitializationHooks);
			...
			(_socketDaemen, _socketRequesterContext.Daemon) =
				MicroserviceAuthenticationDaemon.Start(_args, _requester, _serviceShutdownTokenSource);
		...
		public async Task OnShutdown(object sender, EventArgs args)
		{
			...
			{
				Log.Fatal(
					"Shutting Down with {runningTaskCount} pending task(s) still processing after the {shutdownTimeLimit} second shutdown grace period. This produces inconsistent behaviour. ",
					_runningTaskTable.Count, ShutdownLimitSeconds);
			...
			sw.Stop();
			Log.Information("Service shutting down. Shutdown took {shutdownTime} seconds.",
				sw.ElapsedMilliseconds / 1000f);
		}
		...
		public void RebuildRouteTable()
		{
			...
			ServiceMethods = RouteTableGeneration.BuildRoutes(MicroserviceType, _serviceAttribute, adminRoutes,
		...
		async Task SetupWebsocket(IConnection socket, bool initContent = false)
		{
			...
			try
			{
				...
				HasInitialized = true;
				Log.Information(
					Logs.READY_FOR_TRAFFIC_PREFIX + "baseVersion={baseVersion} executionVersion={executionVersion}",
					_args.SdkVersionBaseBuild, _args.SdkVersionExecution);
		...
		private async Task ResolveCustomInitializationHook()
		{
			...
			serviceInitialization.Sort(delegate((MethodInfo method, InitializeServicesAttribute attr) t1,
				(MethodInfo method, InitializeServicesAttribute attr) t2)
			{
				var (_, attr1) = t1;
				var (_, attr2) = t2;
			...
			foreach (var (initializationMethod, _) in serviceInitialization)
			{
				...
				if (parameters.Length != 1 || parameters[0].ParameterType != typeof(IServiceInitializer))
				{
					BeamableLogger.LogWarning(
						$"Skipping method with [{nameof(InitializeServicesAttribute)}] since it does not take a single [{nameof(IServiceInitializer)}] parameter.");
					continue;
				}
				...
				if (resultType == typeof(void))
				{
					...
					{
						BeamableLogger.LogWarning(
							$"Skipping method [{initializationMethod.DeclaringType?.FullName}.{initializationMethod.Name}] " +
					...
					promise = Task.FromResult(initializationMethod.Invoke(null, new object[] { serviceInitializers }))
						.ToPromise().ToUnit();
				}
				else if (resultType == typeof(Task))
				{
					promise =
						((Task)initializationMethod.Invoke(null, new object[] { serviceInitializers })).ToPromise();
				}
				...
				{
					BeamableLogger.LogWarning(
						$"Skipping method with [{nameof(InitializeServicesAttribute)}] since it isn't a synchronous [void] method, a [{nameof(Task)}] or a [{nameof(Promise<Unit>)}]");
					continue;
				}
				...
				try
				{
					await promise;
					BeamableLogger.Log(
						$"Custom service initializer [{initializationMethod.DeclaringType?.FullName}.{initializationMethod.Name}] succeeded.\n");
				}
				catch (Exception ex)
				{
		...
		Promise<IConnection> AttemptConnection()
		{
			...
			void Attempt()
			{
				...
				ws.OnDisconnect(async (socket, wasClean) =>
				{
					...
					connectionAttempt++;
					Log.Error(
						"connection could not be re-established. Will attempt connection {attempt} in {delay} seconds",
						connectionAttempt, retryDelay);
		...
		async Task HandleClientMessage(MicroserviceRequestContext ctx, Stopwatch sw)
		{
			...
			catch (TargetInvocationException ex)
			{
				...
				var failResponse = new GatewayResponse() { id = ctx.Id, };
				...
				else
				{
					...
					{
						id = ctx.Id, status = 500, body = new ClientResponse { payload = "" }
					};
		...
		{
			Log.Debug("Closing socket connection... clean=[{clean}] isShuttingDown=[{shuttingDown}]", wasClean,
				IsShuttingDown);
			if (!IsShuttingDown)
			{
		...
		{
			var req = new MicroserviceServiceProviderRequest { type = "basic", name = name };
			var serviceProvider = _requester.Request<MicroserviceProviderResponse>(Method.POST, "gateway/provider", req)
				.Then(res =>
				{
		...
		{
			var req = new MicroserviceServiceProviderRequest { type = "basic", name = name };
			var serviceProvider = _requester
				.Request<MicroserviceProviderResponse>(Method.DELETE, "gateway/provider", req).Then(res =>
				{
