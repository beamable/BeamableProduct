...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		if (ctx.args.OtelExporterStandardEnabled)
		{
			...
			if (!status.isRunning)
			{
				...
				if (!existing.HasAll)
				{
					...
					var requester = await GenerateTemporaryRequester(new EnvironmentArgs());
	...
	private static async Task<IBeamableRequester> GenerateTemporaryRequester(EnvironmentArgs args)
	{
		var host = args.Host
			.Replace("ws://", "http://")
			.Replace("wss://", "https://")
		...
		// When we call `project generate-env` running the microservice locally, we don't pass the flag to generate the secret and without the
		// secret, the signed request won't work. This workaround is here to make it use a refresh token instead of the secret.
		// I'm only choosing this approach because I believe that there might be a good reason to not generate the secret locally.
		var tempRequester = new MicroserviceHttpRequester(args, new HttpClient())
		{
			ScopeHeader = args.CustomerID + "." + args.ProjectName
		};
		...
		var authedClient = new HttpClient();
		authedClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {accessToken}");
		return new MicroserviceHttpRequester(args, authedClient)
		{
			ScopeHeader = args.CustomerID + "." + args.ProjectName
		};
	}
