...
namespace Beamable.Server
{
	public interface IMicroserviceAttributes
	{
		...
		/// <summary>
		/// The deployment scope of the service, sourced from the <c>BeamServiceScope</c> csproj property at
		/// build time. Null/empty means realm-scoped. Use <see cref="MicroserviceAttributeExtensions.GetServiceScope"/>
		/// to read this as a <see cref="BeamServiceScope"/>.
		/// </summary>
		string ServiceScope { get; }
	...
	/// <summary>
	/// The deployment scope of a Beamable service. Realm-scoped services run per realm (<c>cid.pid</c>);
	/// zone-scoped services run once per zone (<c>cid.zid</c>), above realms.
	/// </summary>
	public enum BeamServiceScope
	{
		Realm,
		Zone
	}
	...
	{
		/// <summary>
		/// Reads <see cref="IMicroserviceAttributes.ServiceScope"/> as a <see cref="BeamServiceScope"/>,
		/// defaulting to <see cref="BeamServiceScope.Realm"/> when unset or unrecognized.
		/// </summary>
		public static BeamServiceScope GetServiceScope(this IMicroserviceAttributes attributes)
		{
			switch (attributes?.ServiceScope?.Trim().ToLowerInvariant())
			{
				case "zone":
					return BeamServiceScope.Zone;
				default:
					return BeamServiceScope.Realm;
			}
		}
	...
	public class MicroserviceAttribute : Attribute, INamingAttribute, IMicroserviceAttributes
	{
		...
		/// <summary>
		/// Not set via the attribute; service scope is sourced from the <c>BeamServiceScope</c> csproj
		/// property at build time (see <c>BuiltSettings.ReadServiceAttributes</c>).
		/// </summary>
