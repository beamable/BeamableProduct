			...
			ServiceMethodProvider provider)
		{
			...
			foreach (var interfaceType in interfaces)
			{
				...
				for (var i = 0; i < map.TargetMethods.Length; i++)
				{
					...
					Log.Debug("Found Federated method. FederatedPath={FederatedPath}, MethodName={MethodName}", path,
...
#pragma warning restore 618
			...
			ServiceMethodProvider provider)
		{
			...
			foreach (var method in allMethods)
			{
				...
				{
					throw new BeamableMicroserviceException(
						$"The following method is invalid, method=[{type.Name}.{method.Name}]. Callable methods in Beamable Microservices are not allowed to have the `async void` method signature. Consider using `async Promise` or `async Task` instead. ");
				}
				...
				if (output.Select(sm => sm.Path).Contains(servicePath))
					throw new BeamableMicroserviceException(
							$"Overloaded Callables are not currently supported in C#MS! Class={method.DeclaringType.Name} Method={method.Name}")
