using Beamable.Api.Autogenerated.Zones;
...
public partial class BeamoLocalSystem
{
	...
	// in-process ZoneMicroservice reads at boot. Called only when the extension is zone-scoped.
	private async Task ApplyZoneEnvironment(IAppContext appContext, string extensionName)
	{
		var cid = appContext.Cid;
		var localZid = _configService.GetConfigString(ConfigService.CFG_JSON_FIELD_ZID);
		var zid = await ZoneResolver.ResolveZid(_provider, cid, appContext.Pid, localZid);
		if (string.IsNullOrEmpty(zid))
		{
			throw new CliException(
				$"Portal extension [{extensionName}] is zone-scoped (<BeamServiceScope>zone</BeamServiceScope>), " +
				$"but no zone could be resolved. Bind the current realm to a zone, or set a 'zid' in the .beamable " +
				$"config, and retry.");
		}

		var zonesApi = _provider.GetService<IBeamZonesApi>();
		var zoneView = await zonesApi.GetZones(cid, zid);
		var zoneSecret = (zoneView?.secret != null && zoneView.secret.HasValue) ? zoneView.secret.Value : null;
		if (string.IsNullOrEmpty(zoneSecret))
		{
			throw new CliException(
				$"Resolved zone [{zid}] for extension [{extensionName}], but no zone secret was returned. Your " +
				$"account may lack permission to read the zone secret.");
		}

		// NOTE: these are process-global, matching how generate-env publishes env. Running multiple zone
		// extensions concurrently in one `beam project run` would race on them; fine for the common case.
		Environment.SetEnvironmentVariable("BEAM_SKIP_LOCAL_ENV", "true");
		Environment.SetEnvironmentVariable("CID", cid);
		Environment.SetEnvironmentVariable("PID", zid);   // the zone id rides the pid slot
		Environment.SetEnvironmentVariable("ZID", zid);
		Environment.SetEnvironmentVariable("SECRET", zoneSecret);
		Environment.SetEnvironmentVariable("HOST", appContext.Host.Replace("http", "ws") + "/socket");
		Environment.SetEnvironmentVariable("NAME_PREFIX", ServiceRoutingStrategyExtensions.GetDefaultRoutingKeyForMachine());
		// EnvironmentArgs defaults BEAM_INSTANCE_COUNT to 10; a local dev instance wants exactly one (this is
		// what generate-env sets for the realm path). Without it the extension boots 10 instances and spams
		// the "ready for traffic" log.
		Environment.SetEnvironmentVariable("BEAM_INSTANCE_COUNT", "1");
		if (!string.IsNullOrEmpty(appContext.RefreshToken))
		{
			Environment.SetEnvironmentVariable("REFRESH_TOKEN", appContext.RefreshToken);
		}
	}
	...
	private async Task RunMicroserviceForever(BeamoServiceDefinition definition, BeamoLocalSystem localSystem, PortalExtensionConfig config, IAppContext appContext, BeamActivity beamActivity, string portalBaseUrl = null, Action<float, string> onProgress = null, CancellationToken token = default)
	{
		...
		var runtime = new PortalExtensionRuntime(extension.Name);
			...
			// A zone extension boots its backing service as a ZoneMicroservice (cid.zid).
			var isZone = string.Equals(extension.Properties?.ServiceScope?.Trim(), "zone",
				StringComparison.OrdinalIgnoreCase);

			if (isZone)
			{
				// Resolve which zone this extension runs in with the same rule as `beam config`/`generate-env`
				// (a selected realm's bound zone wins; otherwise the .beamable config `zid`), then inject the
				// zone connection env directly. The in-process backing service would otherwise resolve realm
				// env via the nested generate-env, which does not detect a portal extension as zone-scoped.
				// BEAM_SKIP_LOCAL_ENV makes it read these values as-is, mirroring how a standalone zone service
				// boots (PID carries the zone id; auth signs with the zone secret).
				await ApplyZoneEnvironment(appContext, extension.Name);
			}
