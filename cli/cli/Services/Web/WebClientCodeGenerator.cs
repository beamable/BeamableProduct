...
public class WebClientCodeGenerator
{
	...
	// All generation state is per instance: a generator describes exactly one OpenAPI document. Types
	// shared across documents are merged by GenerateClientTypes from the generators passed to it, so
	// nothing leaks between runs in a long-lived process (e.g. the MCP server), between languages, or
	// between concurrent generations (portal extension clients are generated in parallel).
	private readonly string _langType;
	private readonly TsInt64Mapping _int64Mapping;
	// The @beamable/sdk type the generated client binds to: "BeamBase" for realm
	// extensions (Beam/BeamServer), "BeamZoneSdk" for zone extensions. Controls
	// both the client constructor's `beam` param type and which interface the
	// `declare module '@beamable/sdk'` augmentation extends, so `beam.<name>Client`
	// is typed on whichever SDK actually hosts the extension.
	private readonly string _augmentType;
	private readonly TsFile _clientFile;
	private List<string> _modules = new();
	private readonly HashSet<string> _schemas = new();
	// Object types referenced by this document's callables, in discovery order. May contain several
	// aliases with the same name (one per referencing endpoint); GenerateClientTypes collapses them.
	private readonly List<TsTypeAlias> _clientTypes = new();
	...
	/// <param name="document">The microservice's OpenAPI document (its <c>beam_openApi.json</c>).</param>
	/// <param name="langType">The output language: <c>typescript</c>/<c>ts</c> or <c>javascript</c>/<c>js</c>.</param>
	/// <param name="augmentType">The <c>@beamable/sdk</c> type the client binds to.</param>
	/// <param name="int64Mapping">How C# <c>long</c> (OpenAPI <c>int64</c>) fields are typed in TypeScript.</param>
	public WebClientCodeGenerator(OpenApiDocument document, string langType, string augmentType = "BeamBase",
		TsInt64Mapping int64Mapping = TsInt64Mapping.BigIntUnion)
	{
		_langType = langType ?? "typescript";
		_int64Mapping = int64Mapping;
		_augmentType = string.IsNullOrWhiteSpace(augmentType) ? "BeamBase" : augmentType;
		...
		var tsBeamConstructorParams = new TsConstructorParameter("beam", TsType.Of(_augmentType), !IsTypeScript);
		...
		if (IsTypeScript)
		{
			GenerateBeamSDKModuleDeclaration(_clientFile, tsClass, _augmentType);
		}
		...
		tsBeamSdkImport.AddNamedImport($"type {_augmentType}");
		...
		foreach ((string path, OpenApiPathItem item) in document.Paths)
		{
			...
			foreach ((OperationType _, OpenApiOperation operation) in item.Operations)
			{
				...
				if (response.Content.TryGetValue("application/json", out var responseMediaType))
				{
					...
					{
						responseType = OpenApiTsTypeMapper.Map(responseSchema, ref _modules, _int64Mapping);
				    ...
				    false)
				{
					...
					{
						requestType = OpenApiTsTypeMapper.Map(requestSchema, ref _modules, _int64Mapping);
		...
		List<string> modules, bool isBeamSchemaRequest, bool isBeamSchemaResponse)
	{
		...
		// An array response (e.g. `MatchView[]`) references its element type, which lives in the types file too.
		var returnElementTypeRender = returnTypeRender.EndsWith("[]")
			? returnTypeRender.Substring(0, returnTypeRender.Length - 2)
			: returnTypeRender;
		TsType returnTypePromise = modules.Exists(m => returnElementTypeRender == m)
	...
	private void HandleSchema(OpenApiSchema schema, TsType type)
	{
		...
		foreach ((string propName, OpenApiSchema propSchema) in schemaProps)
		{
			...
			var propType = OpenApiTsTypeMapper.Map(propSchema, ref modules, _int64Mapping);
		...
		_clientTypes.Add(new TsTypeAlias(typeRender)
			.AddModifier(TsModifier.Export)
			.SetType(TsType.Object(objectPropTypes.ToArray())));
	}
	...
	private static void GenerateBeamSDKModuleDeclaration(TsFile clientFile, TsClass tsClass, string augmentType)
	{
		...
		var tsInterface = new TsInterface(augmentType);
	...
	/// <summary>
	/// The object types this document's callables reference, which belong in the shared <c>types</c> file.
	/// </summary>
	public IReadOnlyList<TsTypeAlias> ClientTypes => _clientTypes;

	/// <summary>
	/// Writes <c>index.ts</c> into <paramref name="typesOutputDirectory"/> with the client types of every given
	/// generator, each type name emitted once.
	/// </summary>
	/// <returns>The written file path, or an empty string when there are no types (nothing is written).</returns>
	public static string GenerateClientTypes(string typesOutputDirectory, IEnumerable<WebClientCodeGenerator> generators)
	{
		...
		// callables, or by two microservices) is collected once per referencing endpoint, which would
		// emit duplicate `export type` declarations that fail to compile. Collapse by type name,
		// keeping the first occurrence.
		var seenTypeNames = new HashSet<string>();
		foreach (var generator in generators)
		{
			foreach (var clientType in generator._clientTypes)
			{
	...
	public bool IsTypeScript => IsTypeScriptLanguage(_langType);

	public static bool IsTypeScriptLanguage(string langType) =>
		string.Equals(langType, "typescript", StringComparison.InvariantCultureIgnoreCase) ||
		string.Equals(langType, "ts", StringComparison.InvariantCultureIgnoreCase);
}
