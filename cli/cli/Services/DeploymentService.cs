...
// The realm manifest (ManifestView) now speaks the BeamoBasic* type family after the beamo-bundle
// OpenAPI regen. These aliases keep the merge/diff/build pipeline below unchanged while pointing the
// short names at the new types (and force the Models types over cli.Services.BeamoService's own
// same-named classes).
using ServiceReference = Beamable.Api.Autogenerated.Models.BeamoBasicServiceReference;
using ServiceStorageReference = Beamable.Api.Autogenerated.Models.BeamoBasicServiceStorageReference;
using ServiceComponent = Beamable.Api.Autogenerated.Models.BeamoBasicServiceComponent;
using ServiceDependencyReference = Beamable.Api.Autogenerated.Models.BeamoBasicServiceDependencyReference;
using OptionalArrayOfServiceStorageReference = Beamable.Api.Autogenerated.Models.OptionalArrayOfBeamoBasicServiceStorageReference;
using OptionalArrayOfServiceComponent = Beamable.Api.Autogenerated.Models.OptionalArrayOfBeamoBasicServiceComponent;
using OptionalArrayOfServiceDependencyReference = Beamable.Api.Autogenerated.Models.OptionalArrayOfBeamoBasicServiceDependencyReference;
...
/// <summary>
/// CLI-side representation of a portal extension entry in a deployment plan.
/// Uses plain types to avoid pulling autogenerated API model types (e.g. BeamoV2PortalExtensionReference)
/// into the command output type graph, which would fail the StreamingDataTests assembly check.
/// </summary>
public class PortalExtensionPlanReference : JsonSerializable.ISerializable
{
	public string name;
	public string checksum;
	public bool enabled;
	public bool archived;
	public List<PortalExtensionPlanFileReference> files = new List<PortalExtensionPlanFileReference>();

	public void Serialize(JsonSerializable.IStreamSerializer s)
	{
		s.Serialize(nameof(name), ref name);
		s.Serialize(nameof(checksum), ref checksum);
		s.Serialize(nameof(enabled), ref enabled);
		s.Serialize(nameof(archived), ref archived);
		s.SerializeList(nameof(files), ref files);
	}
	...
	public static PortalExtensionPlanReference FromBeamoV2(BeamoV2PortalExtensionReference r) => new PortalExtensionPlanReference
	{
		...
		checksum = r.checksum.GetOrElse(""),
		enabled = r.enabled.GetOrElse(true),
		archived = r.archived.GetOrElse(false),
		files = r.files.GetOrElse(Array.Empty<BeamoV2ExtensionContentReference>())
	...
	public static BeamoV2PortalExtensionReference ToBeamoV2(PortalExtensionPlanReference r) => new BeamoV2PortalExtensionReference
	{
		...
		files = r.files.Count == 0
			? new OptionalArrayOfBeamoV2ExtensionContentReference()
			: new OptionalArrayOfBeamoV2ExtensionContentReference
			{
				HasValue = true,
				Value = r.files.Select(f => new BeamoV2ExtensionContentReference
				{
...
public class DeployablePlan : JsonSerializable.ISerializable
{
	...
	public List<PortalExtensionUploadInfo> portalExtensionsToUpload = new List<PortalExtensionUploadInfo>();
	public List<PortalExtensionPlanReference> portalExtensionReferences = new List<PortalExtensionPlanReference>();
	...
	public void Serialize(JsonSerializable.IStreamSerializer s)
	{
		...
		s.SerializeList(nameof(portalExtensionsToUpload), ref portalExtensionsToUpload);
		s.SerializeList(nameof(portalExtensionReferences), ref portalExtensionReferences);
		s.SerializeDictionary(nameof(references), ref references);
	}
}
...
public class DeploymentDiffSummary : JsonSerializable.ISerializable
{
	...
	public List<string> addedPortalExtensions = new List<string>();
...
public partial class DeployUtil
{
	...
	/// <summary>
	/// The realm manifest can carry several entries for the same name — archived history alongside the
	/// current live entry. Collapse to a single canonical entry per name: the non-archived (live) entry
	/// when one exists, otherwise the first archived entry seen. First-appearance order is preserved so
	/// that a collapsed manifest used to seed <see cref="MergeAdditive(ManifestView, ManifestView)"/>
	/// keeps the positional array diff in <see cref="FindChanges"/> index-aligned.
	/// </summary>
	public static List<T> CollapseByName<T>(IEnumerable<T> items, Func<T, string> nameOf, Func<T, bool> isArchived)
	{
		...
		foreach (var item in items)
		{
			...
			// prefer the live (non-archived) entry over an archived duplicate.
			if (isArchived(existing) && !isArchived(item))
			{
				byName[name] = item;
			}
		}
	...
	/// <summary>
	/// Resolves the component ids (services + storages + portal extensions) of every referenced bundle in
	/// <paramref name="pins"/> so the caller can exclude them from the realm's inline diff — a component
	/// delivered by a bundle reaches the realm via the bundle, not as an inline component, whether or not
	/// the bundle is authored locally. The realm's already-assembled <paramref name="remoteV2"/>.bundles
	/// lists components for bundles it currently pins (no network call); anything else (a newly-pinned
	/// bundle, or one authored by another customer such as <c>@beamable/defaults</c>) is resolved from the
	/// catalog. Best-effort: a per-bundle resolution failure is logged and skipped so it can't break the
	/// plan (its components simply fall back to appearing as inline entries).
	/// </summary>
	private static async Task<HashSet<string>> ResolvePinnedBundleComponentIds(
		IDependencyProvider provider, IDictionary<string, string> pins, BeamoV2Manifest remoteV2)
	{
		...
		var assembled = remoteV2?.bundles.GetOrElse(Array.Empty<BeamoV2ManifestBundle>())
		                ?? Array.Empty<BeamoV2ManifestBundle>();
		IBeamBeamobundleApi bundleApi = null;
		...
		foreach (var kvp in pins)
		{
			...
			// fast path: the realm already resolved this exact bundle pin, so its component ids are on hand.
			var match = assembled.FirstOrDefault(b =>
				b.name.GetOrElse("") == fullName && b.checksum.GetOrElse("") == checksum);
			if (match != null)
			{
				foreach (var id in match.services.GetOrElse(Array.Empty<string>()))
				{
					ids.Add(id);
				}
				foreach (var id in match.storages.GetOrElse(Array.Empty<string>()))
				{
					ids.Add(id);
				}
				foreach (var id in match.portalExtensions.GetOrElse(Array.Empty<string>()))
				{
					ids.Add(id);
				}
				continue;
			...
			try
			{
				var (ns, name) = BundleNamespace.SplitName(fullName);
				bundleApi ??= provider.GetService<IBeamBeamobundleApi>();
				var response = await bundleApi.GetBundlesChecksums(name, checksum, ns);
				var info = BundleInfo.FromBundle(response.bundle);
				if (info != null)
				{
					foreach (var id in info.serviceNames)
					{
						ids.Add(id);
					}
					foreach (var id in info.storageIds)
					{
						ids.Add(id);
					}
					foreach (var id in info.portalExtensionNames)
					{
						ids.Add(id);
					}
				}
			...
			{
				Log.Warning($"Could not resolve components for pinned bundle [{fullName}] ({checksum}); " +
				            $"its components may appear as inline changes. {e.Message}");
			}
		}
	...
	/// <summary>
	/// Diffs portal-extension references and pinned bundle references between the realm's current state
	/// (<paramref name="remotePortalExtensionRefs"/> / <paramref name="remoteBundleReferences"/>) and the
	/// merged intended state (<paramref name="nextPortalExtensionRefs"/> / <paramref name="nextBundleReferences"/>),
	/// appending the results to <paramref name="diff"/>. These aren't part of the v1 ManifestView, so
	/// <see cref="FindChanges"/> can't see them. Callers must pass a remote set already collapsed to one
	/// canonical entry per name (see <see cref="CollapseByName{T}"/>) so a live entry isn't compared
	/// against a stale archived duplicate and reported as a phantom change.
	/// </summary>
	public static void DiffReferences(
		List<PortalExtensionPlanReference> remotePortalExtensionRefs,
		List<PortalExtensionPlanReference> nextPortalExtensionRefs,
		IDictionary<string, string> remoteBundleReferences,
		IDictionary<string, string> nextBundleReferences,
		DeploymentDiffSummary diff)
	{
		...
		// authored bundle (see the bundle-component handling in Plan).
		foreach (var remoteRef in remotePortalExtensionRefs)
		{
			if (remoteRef.archived)
			{
				continue;
			}
			if (nextPortalExtensionRefs.Any(r => r.name == remoteRef.name))
			{
				continue;
			}
 ...
 - or run the `dotnet beam deploy release` command to create a plan and release it sequentially. ");
	...
	public static void PrintPlanInfo(DeployablePlan plan, IHasDeployPlanArgs args, out bool hasChanges)
	{
		...
		// Only surface the post-release bundle pins when the plan actually changes something; otherwise
		// this reads as a pending change when the summary already said "There are no changes."
		if (hasChanges && plan.references?.Count > 0)
		{
	...
	where TArgs : CommandArgs, IHasDeployPlanArgs
	{
		...
		// The realm's current manifest can carry several entries for the same name — archived history
		// alongside the current live entry. Collapse to one canonical (live-preferred) entry per name
		// BEFORE the merge/diff. FindChanges diffs the service/storage arrays positionally by index, and
		// `next` is built from `remote` via MergeAdditive, so deduping here keeps the two index-aligned;
		// otherwise a live entry gets compared against a stale archived duplicate and reports a phantom change.
		remote.manifest = CollapseByName(remote.manifest ?? Array.Empty<BeamoBasicServiceReference>(),
			s => s.serviceName, s => s.archived).ToArray();
		if (remote.storageReference.HasValue && remote.storageReference.Value != null)
		{
			remote.storageReference.Value = CollapseByName(remote.storageReference.Value,
				s => s.id, s => s.archived).ToArray();
		}
		...
		// The realm manifest can carry several entries per extension name (archived history + the live
		// one). Collapse to a single canonical entry per name (live preferred) so an unchanged extension
		// isn't reported as "changed" merely because it's compared against a stale archived duplicate.
		var remotePortalRefs = CollapseByName(
				beamoV2Manifest.portalExtensionReferences.GetOrElse(Array.Empty<BeamoV2PortalExtensionReference>()),
				r => r.name.GetOrElse(""), r => r.archived.GetOrElse(false))
			.ToArray();
		...
		// A component delivered by a referenced bundle reaches the realm via the bundle, not as an inline
		// component, so it must not appear in the inline diff at all — only the bundle reference does.
		// `bundleComponentIds` starts from locally-authored bundles; extend it with the components of every
		// pinned bundle in the final reference set (resolved from the realm's assembled view or the catalog)
		// so bundles we don't author locally (e.g. @beamable/defaults) are covered too. Then strip those
		// components from BOTH the remote and the intended manifests: symmetric removal means they neither
		// show as inline changes (add/change/remove/disable) nor get posted as inline entries — the server
		// assembles them from the bundle references instead.
		...
		if (bundleComponentIds.Count > 0)
		{
			...
			if (remote.storageReference.HasValue && remote.storageReference.Value != null)
			{
				remote.storageReference.Value = remote.storageReference.Value
					.Where(st => !bundleComponentIds.Contains(st.id)).ToArray();
			}
			if (next.storageReference.HasValue && next.storageReference.Value != null)
			{
				next.storageReference.Value = next.storageReference.Value
					.Where(st => !bundleComponentIds.Contains(st.id)).ToArray();
			}
			remotePortalExtensionRefs = remotePortalExtensionRefs
		...
		DiffReferences(remotePortalExtensionRefs, nextPortalExtensionRefs, remoteBundleReferences, nextBundleReferences, diff);
		...
		int maxConcurrentUploads=6)
	{
		...
		// plan.portalExtensionReferences now has fully populated files[] for every uploaded PE
		var beamoV2PostManifestRequest = new BeamoV2PostManifestRequest
		{
			...
			portalExtensionReferences = new OptionalArrayOfBeamoV2PortalExtensionReference
			{
				...
				Value = plan.portalExtensionReferences.Select(PortalExtensionPlanReference.ToBeamoV2).ToArray(),
			},
			autoDeploy = true,
	...
	private static async Task UploadToSignedUrl(HttpClient httpClient, string uploadMethod, string url, byte[] data, string contentType, byte[] md5Bytes)
	{
		...
		if (!response.IsSuccessStatusCode)
		{
			...
			throw new CliException($"Failed to upload portal extension binary to signed URL. Status={response.StatusCode}, Body={body}");
		}
	}
