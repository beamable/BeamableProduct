...
{
	public class OpenApiServerCodeGenerator
	{
		...
		public OpenApiServerCodeGenerator(OpenApiDocument document)
		{
			...
			{
				ReturnType = new CodeTypeReference("BeamServiceConfigBuilder"),
				Attributes = MemberAttributes.Public | MemberAttributes.Final | MemberAttributes.Static
			};
			registrationMethod.Name = $"With{TargetClassName}";
			registrationMethod.Parameters.Add(
				new CodeParameterDeclarationExpression("this BeamServiceConfigBuilder", "builder"));

			registrationMethod.Statements.Add(new CodeSnippetStatement("builder.UseRealmSecretAuth();"));
			registrationMethod.Statements.Add(
				new CodeSnippetStatement($"builder.ConfigureServices(b => b.AddScoped<{TargetClassName}>());"));
			registrationMethod.Statements.Add(new CodeSnippetStatement("return builder;"));

			
			// public static SalmonClient2 SalmonClient(this IBeamableServices services)
			// {
			// 	var provider = new DependencyProvider();
			// 	return provider.InstantiateService<SalmonClient2>();
			// }
			var extensionMethod = new CodeMemberMethod
			{
				Name = TargetClassName,
				ReturnType = new CodeTypeReference(TargetClassName), 
				Attributes = MemberAttributes.Public | MemberAttributes.Final | MemberAttributes.Static,
				Parameters =
				{
					new CodeParameterDeclarationExpression("this IBeamableServices", "services")
				}, 
				Statements =
				{
					new CodeSnippetStatement("var provider = services.Scope;"),
					new CodeSnippetStatement($"var client = provider.InstantiateService<{TargetClassName}>();"),
					new CodeSnippetStatement("return client;"),
				}
			};
		...
		private static CodeNamespace GenerateNamespace()
		{
			...
			newNamespace.Imports.Add(new CodeNamespaceImport("Beamable.Common.Dependencies"));
			...
			Dictionary<string, string> paramsTypeName)
		{
			...
			string genericPromiseType = $"System.Threading.Tasks.Task<{returnTypeString}>";
