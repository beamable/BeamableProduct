...
public partial class BeamoLocalSystem
{
	...
	public ContainerUploadData PrepareContainerUploader(string cid, string gamePid, string realmPid, string token, string beamoId, string dockerRegistryUrl, BeamoServiceDefinition beamoService)
	{
		...
		containerUploadData.Md5 = MD5.Create();
		containerUploadData.Client = new HttpClient();
	...
	protected string GetHash(string input, MD5 md5)
	{
	...
	public async Task Upload(ContainerUploadData data, string folder, CancellationToken token, Action<string, float> onContainerUploadProgress = null)
	{
		...
		{
			{ "schemaVersion", 2 }, { "mediaType", MediaManifest }, { "config", new Dictionary<string, object> { { "mediaType", MediaConfig } } }, { "layers", new List<object>() },
		};
	...
	private async Task<FileBlobResult> UploadFileBlob(ContainerUploadData data, string filename, CancellationToken token)
	{
		token.ThrowIfCancellationRequested();
	...
	private async Task<HttpResponseMessage> UploadChunk(ContainerUploadData data, FileChunk chunk, Uri location, CancellationToken token)
	{
		var uri = location;
	...
	private async Task<Uri> PrepareUploadLocation(ContainerUploadData data)
	{
	...
	private string HashDigest(HashAlgorithm _sha256, Stream stream)
	{
