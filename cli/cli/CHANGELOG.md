...
- `beam config realm check --for web [--fix]` checks that a realm is ready for the Web SDK: the `beamable` realtime publisher, a published `global` content manifest, running deployed services and a reachable realtime socket host. Results are a structured stream (with the fix command for each failed check), the command exits non-zero when a check fails, and `--fix` sets the publisher and publishes the `global` manifest.
- New skills for AI agents: `beam-web-game-quickstart` (the end-to-end path from `beam init` to a deployed, hosted web game), `beam-test-and-verify` (calling endpoints as a player, probing the realtime session, logs, storage and a browser smoke test) and `beam-web-hosting` (static hosting on GitHub Pages, what is safe to ship, CORS).
...
- `beam listen player --probe` opens one realtime session the way the Web SDK does (refresh-token exchange for the socket JWT, `websocketConfig.uri` from the realm's client defaults, `send-session-start=true` plus the `session-start` frame), listens for `--probe-seconds` (default 5), and reports the handshake status, time to open, frames received and any server close on the `probe` channel. It exits 1 with an actionable message when the handshake is rejected, the socket is still connecting after 15 seconds, or the server closes it. The probe signs in as a new guest unless `--refresh-token` is passed; `--guest` makes plain `listen player` use a new guest too.
- `beam project call <service> <method> [--payload <json>] [--as guest|<refresh-token>] [--target auto|local|remote]` calls a microservice endpoint as a player and returns the HTTP `status`, the response `body`, the caller's `playerId` and a `refreshToken` to pass to `--as` so the next call is made by the same player. By default it creates a new guest and calls a locally running instance of the service when one is discovered, otherwise the deployed one. A non-2xx response is still reported, but the command exits non-zero.
