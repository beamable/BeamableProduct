...
public class ServiceInstance
{
	...
	public RemotePortalExtensionDescriptor latestRemotePortalExtensionEvent;
}
...
public class CheckStatusCommand : StreamCommand<CheckStatusCommandArgs, CheckStatusServiceResult>
{
	...
	void Report(CheckStatusServiceResult update)
	{
		...
		void AddRow(ServiceStatus service, ServicesForRouteCollection route, ServiceInstance instance)
		{
			...
			if (instance?.latestRemotePortalExtensionEvent != null)
			{
				var ext = instance.latestRemotePortalExtensionEvent;
				infoValue = ext.archived ? "archived" : ext.enabled ? "enabled" : "disabled";
				if (ext.dependencies.Length > 0)
				{
					infoValue += $",deps={string.Join(";", ext.dependencies)}";
				}
			}
			...
			if (instance?.latestRemotePortalExtensionEvent != null) originValue = "remote";
			table.AddRow(
				service.service,
				service.serviceType,
				originValue,
		...
		[EnumeratorCancellation] CancellationToken token = default)
	{
		...
		// Fetch deployed portal extensions from the V2 manifest
		try
		{
			var beamBeamoApi = args.Provider.GetService<IBeamBeamoApi>();
			var v2Manifest = await beamBeamoApi.GetManifestsCurrent();
			if (v2Manifest.portalExtensionReferences.TryGet(out var extensions))
			{
				foreach (var ext in extensions)
				{
					var name = ext.name.GetOrElse("");
					if (string.IsNullOrEmpty(name)) continue;

					var isEnabled = ext.enabled.GetOrElse(true);
					var isArchived = ext.archived.GetOrElse(false);
					var isRunning = isEnabled && !isArchived;

					var remoteInstance = new ServiceInstance
					{
						primaryKey = $"remote-portal-extension-{name}",
						startedByAccountEmail = "",
						latestRemotePortalExtensionEvent = new RemotePortalExtensionDescriptor
						{
							name = name,
							enabled = isEnabled,
							archived = isArchived,
							dependencies = ext.dependencies.GetOrElse(Array.Empty<string>()),
						}
					};

					var remoteRoute = new ServicesForRouteCollection
					{
						knownToBeRunning = isRunning,
						routingKey = "",
						instances = new List<ServiceInstance> { remoteInstance },
						federations = new List<FederationInstance>(),
					};

					if (!result.TryGetStatus(name, out var peStatus))
					{
						peStatus = new ServiceStatus
						{
							service = name,
						...
						result.services.Add(peStatus);
					}
					else
					{
						peStatus.availableRoutes.Add(remoteRoute);
					}
				}
			}
			}
		...
		{
			Log.Debug("Failed to fetch V2 manifest for portal extensions: {Error}", ex.Message);
		}
			...
			if (!result.TryGetStatus(discoveryEvent.Service, out var status))
			{
				status = new ServiceStatus
				{
					...
					groups = discoveryEvent switch
					{
						...
						RemotePortalExtensionEvent => Array.Empty<string>(),
			...
			if (!status.TryGetRoutes(discoveryEvent.RoutingKey, out var collection))
			{
				collection = new ServicesForRouteCollection
				{
					...
					federations = discoveryEvent switch
					{
						...
						RemotePortalExtensionEvent => new List<FederationInstance>(),
			...
			if (!collection.TryGetInstance(discoveryEvent.PrimaryKey, out var instance))
			{
				...
				switch (discoveryEvent)
				{
					...
					case RemoteStorageEvent remoteStorageEvt:
						...
						break;
					case RemotePortalExtensionEvent remotePortalExtensionEvt:
						instance.latestRemotePortalExtensionEvent = remotePortalExtensionEvt.descriptor;
						break;
				}
