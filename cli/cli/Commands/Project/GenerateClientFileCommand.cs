...
public class GenerateClientFileCommand : AppCommand<GenerateClientFileCommandArgs>
{
		...
		foreach (var type in allTypes)
		{
			...
			if (args.outputToLinkedProjects)
			{
				...
				foreach (var unityProjectPath in linkedUnityProjects)
				{
					...
					try
					{
						Task generationTask = GenerateUnityFile(generator, descriptor, args, unityAssetPath);
				...
				foreach (var unrealProjectPath in linkedUnrealProjects)
				{
					...
					try
					{
						Task generationTask = GenerateUnrealFile(unrealFileDescriptors, args, unrealAssetPath);
	...
	Task GenerateUnityFile(ClientCodeGenerator generator, MicroserviceDescriptor descriptor, GenerateClientFileCommandArgs args, string projectPath)
	{
	...
	Task GenerateUnrealFile(List<GeneratedFileDescriptor> descriptors, GenerateClientFileCommandArgs args, string projectPath)
	{
		if (!Directory.Exists(projectPath))
		{
			throw new DirectoryNotExistException();
