...
```
This builds Docker images for all services and generates a deployment plan. Key options:
- `--comment \"<text>\"` — attach a comment to the deployment
- `--ids <service1> <service2>` — target specific services
- `--build-sequentially` — build one at a time instead of parallel
- `--merge` — add services to what's already deployed (additive)
- `--replace` — replace all deployed services (default)
