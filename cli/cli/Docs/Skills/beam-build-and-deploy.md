...
4. **How to verify**: The user can run `deploy list` to see the current deployment state, or check individual service health via the Beamable Portal. If something went wrong, `deploy plan` output contains build logs and image details. Also, offers the User a way to open the portal using the beam command `portal`
