...
namespace Beamable.Common.Scheduler
{
	...
	public class HttpAction : ISchedulableAction
	{
		...
		{
			var headers = new MapOfString();
			foreach (var header in headers)
			{
				headers.Add(header.Key, header.Value);
			}

			var headersOptional = new OptionalMapOfString();
			headersOptional.SetValue(headers);
			return new HttpCall
			{
	...
	public class ServiceAction : ISchedulableAction
	{
		...
		IOneOf_HttpCallOrPublishMessageOrServiceCall ISchedulableAction.Convert()
		{
			...
			{
				headers.Add(Constants.Requester.HEADER_ROUTINGKEY,value);
			}
			var headersOptional = new OptionalMapOfString();
			headersOptional.SetValue(headers);
...
namespace Beamable.Api.Autogenerated.Models
{
	...
	public partial class HttpCall
	{
		public ISchedulableAction Convert()
		{
			...
			return new HttpAction
			{
				...
				method = parsedMethod,
				headers = headers.GetOrElse(new MapOfString()).Select(v => new HttpCallHeader
				{
					key = v.Key,
					value = v.Value,
				}).ToList()
	...
	public partial class ServiceCall
	{
		public ISchedulableAction Convert()
		{
			...
			if (headers.TryGet(out var map) && map.TryGetValue(Constants.Requester.HEADER_ROUTINGKEY, out string value))
			{
				routingKey.SetValue(value);
			}
