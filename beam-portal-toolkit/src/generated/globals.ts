...
declare global {
  ...
  interface BeamPaginationElement extends HTMLElement {
    ...
    /** Total number of pages, clamped to at least 1. */
    totalPages?: number;
    /** First (1-indexed) item shown on the current page. Returns 0 when total is 0. */
    rangeStart?: number;
    /** Last (1-indexed) item shown on the current page. */
    rangeEnd?: number;
  }
  ...
  interface BeamChangeBarElement extends HTMLElement {
    /** The change set to render. Bar is hidden when all three lists are empty. @default {} */
    changes?: unknown;
    /** Validation errors keyed by `entry.key`. When non-empty, Save is disabled. @default [] */
    errors?: unknown;
