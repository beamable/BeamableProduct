...
#
# Usage:
#   ./install-beamplatformnotifications.sh --source <beam-native-mobile> [--project <dir>]
#   ./install-beamplatformnotifications.sh --source <beam-native-mobile> --generate-only <outDir>
#
#   --source <dir>          Path to the BeamableProduct/beam-native-mobile checkout (required).
...
[[ -n "$SOURCE" ]] || die "Missing --source <beam-native-mobile>."
...
MASTER="$SOURCE/Unreal"                                    # master plugin (sources + Scripts + ThirdParty + docs)
NSE_SRC="$SOURCE/NativeSources/iOS/BeamableNotifications/extension"
