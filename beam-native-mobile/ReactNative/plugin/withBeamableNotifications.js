 ...
 *
 * Config (app.json) — reference this package by name (Expo resolves its app.plugin.js):
 *   ["@beamable/notifications-react-native", { "appGroup": "group.com.beamable.rnsample", "enableServiceExtension": true }]
 *
...
// Config plugins always run inside a project that has `expo` (which provides
// `@expo/config-plugins`). Resolving from the project root makes this work even when the
// package is a symlinked `file:`/monorepo dependency (Node would otherwise resolve from the
// package's realpath, where `@expo/config-plugins` isn't reachable). Falls back to normal
// resolution for a conventionally-installed (hoisted) package.
...
// `file:` consumers with always-fresh sources). Both use the same subpath layout.
const VENDORED_IOS = path.join(__dirname, 'ios');
const SDK_ROOT = fs.existsSync(path.join(VENDORED_IOS, 'extension'))
  ? VENDORED_IOS
  : path.resolve(__dirname, '../../NativeSources/iOS/BeamableNotifications');
...
function withReceivedHandlerSource(config) {
  return withDangerousMod(config, [
    ...
    (cfg) => {
      ...
      // The default handler template ships inside this package (plugin/android/).
